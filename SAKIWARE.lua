...
-- Load config now — all UI elements are registered at this point
sakiConfig:Load()

-- After Load(), WindUI restores element visuals but does NOT fire callbacks.
-- So we manually start anything that loaded as enabled.
task.defer(function()
    -- Stamina
    if stam.on    then stamStart()  end
    if stam.noLoss then stam.on = true; stamStart(); stamApply() end
    -- Speed
    if speedHack.on then speedStart() end
    -- Anti-Backstab
    if abs.on then absStart() end
    -- Aimbot (General) — event-driven, flag being true is enough, no explicit start needed
    -- Combat (Guest 1337)
    if combatS.autoBlockOn then combatSetupSoundWatcher(); combatStartLoops() end
    if combatS.killerCircles then combatStartLoops() end
    if combatS.facingVisual then combatStartLoops() end
    if combatS.aimPunchActive and lp.Character then combatSetupAimPunch(lp.Character) end
    -- ESP
    if esp.killers    then task.spawn(function() espDoKillers(true)    end) end
    if esp.survivors  then task.spawn(function() espDoSurvivors(true)  end) end
    if esp.generators then task.spawn(function() espDoGenerators(true) end) end
