...
do
    ...
    -- Attack remotes fired when within 13 studs
    local AI_ATTACK_RANGE   = 13
    local AI_ATTACK_CD      = 4.0 -- seconds between attacks
    local ai_lastAttack     = 0
    local ai_attackIndex    = 1
    local ai_attackRemote   = nil

    local ai_attacks = {
        buffer.fromstring("\x03\x05\x00\x00\x00Punch"),
        buffer.fromstring("\x03\x05\x00\x00\x00Slash"),
    }

    local function aiGetRemote()
        if ai_attackRemote and ai_attackRemote.Parent then return ai_attackRemote end
        local ok, re = pcall(function()
            return svc.RS.Modules.Network.Network:FindFirstChild("RemoteEvent")
        end)
        if ok and re then ai_attackRemote = re end
        return ai_attackRemote
    end

    local function aiFireAttack()
        local now = tick()
        if now - ai_lastAttack < AI_ATTACK_CD then return end
        ai_lastAttack = now
        local re = aiGetRemote(); if not re then return end
        local buf = ai_attacks[ai_attackIndex]
        ai_attackIndex = (ai_attackIndex % #ai_attacks) + 1
        pcall(function()
            re:FireServer("UseActorAbility", { buf })
        end)
    end
    ...
    local function aiKillerLoop()
        while ai_enabled do
            ...
            if targetHRP then
                ...
                -- Within attack range — fire remotes
                if dist <= AI_ATTACK_RANGE then
                    aiFireAttack()
                else
                    ...
                    if ok and path.Status == Enum.PathStatus.Success then
                        for _, wp in ipairs(path:GetWaypoints()) do
                            ...
                            -- Re-check distance each waypoint step so attacks fire ASAP
                            local tgt2 = getNearestSurvivor()
                            if tgt2 and (tgt2.Position - hrp.Position).Magnitude <= AI_ATTACK_RANGE then
                                aiFireAttack(); break
                            end
