...
-- ============================================================
-- SHARED AUTOROTATE MANAGER
-- Prevents multiple systems (aimbot, aimPunch, elliot, HDT)
-- from clobbering each other's AutoRotate save/restore.
-- Any system that wants to disable AutoRotate calls arLock(key)
-- and arUnlock(key) when done. The true original value is saved
-- once on the first lock and only restored when ALL locks release.
-- ============================================================
local arManager = {
    locks   = {},   -- set of active lock keys
    trueVal = nil,  -- the real pre-any-lock AutoRotate value
}

local function arLock(key, hum)
    if not hum or not hum.Parent then return end
    if not next(arManager.locks) then
        -- first lock: save the real value
        arManager.trueVal = hum.AutoRotate
    end
    arManager.locks[key] = true
    hum.AutoRotate = false
end

local function arUnlock(key, hum)
    arManager.locks[key] = nil
    if not next(arManager.locks) then
        -- all locks released: restore
        if arManager.trueVal ~= nil and hum and hum.Parent then
            hum.AutoRotate = arManager.trueVal
        end
        arManager.trueVal = nil
    end
end

local function arForceRelease(hum)
    -- call on character death / respawn to reset all state
    arManager.locks = {}
    if arManager.trueVal ~= nil and hum and hum.Parent then
        hum.AutoRotate = arManager.trueVal
    end
...
local function aimUnlock()
    pcall(function()
        ...
        arUnlock("aim", aim.hum)
        aim.targeting=false; aim.target=nil; aim.autoRotate=nil
    end)
end
...
local function aimLock(r)
    pcall(function()
        ...
        aimUnlock(); aim.target=r; aim.targeting=true
        arLock("aim", aim.hum)
...
-- ============================================================
-- GROUNDBULB & VINE ESP - FIXED WITH SCAN LOOPS
-- ============================================================
...
-- Function to find the highest part or flower part of the model
...
-- Function to highlight a specific target model
local function gvHighlightEntity(model, displayName)
    if not model or not model:IsA("Model") then return end
    if gvEsp.trackedEntities[model] then return end -- Already tracked
    
    -- FILTER: Ignore models with Humanoid (players)
...
local function gvEvaluateAndHighlight(child)
    ...
    -- FILTER: IGNORE MAP AND PLAYER FOLDERS
    if child.Name == "Map" then return end
    if child.Name == "Players" then return end
    ...
    if isGroundBulb and gvEsp.showGroundbulb and not gvEsp.trackedEntities[child] then
        gvHighlightEntity(child, "GroundBulb")
    elseif isVine and gvEsp.showVine and not gvEsp.trackedEntities[child] then
        gvHighlightEntity(child, "Vine")
    end
end

-- Scan workspace for Vines and GroundBulbs
...
-- Hide all tracked highlights without clearing the table
local function gvHideAll()
    for _, data in pairs(gvEsp.trackedEntities) do
        pcall(function()
...
-- Show all tracked highlights that match current show flags
local function gvShowAll()
    for model, data in pairs(gvEsp.trackedEntities) do
        pcall(function()
            local nameLower = model.Name:lower()
            local isVine = nameLower:find("vine")
            local isGB   = nameLower:find("groundbulb")
            local show = (isVine and gvEsp.showVine) or (isGB and gvEsp.showGroundbulb)
...
local function gvStart()
    gvEsp.on = true
    gvShowAll()
    print("[GV-ESP] Enabled")
end

local function gvStop()
    gvEsp.on = false
    gvHideAll()
    print("[GV-ESP] Disabled")
end

-- ALWAYS-RUNNING Heartbeat for continuous scanning
svc.Run.Heartbeat:Connect(function()
    gvCleanupEntities()
    if gvEsp.on then gvScanForSpawns() end
end)

-- ChildAdded listener for new entities
task.spawn(function()
    local lastIngame = nil
    while true do
        local map = svc.WS:FindFirstChild("Map")
        local ingame = map and map:FindFirstChild("Ingame")
        if ingame and ingame ~= lastIngame then
            lastIngame = ingame
...
task.delay(2, function()
    gvScanForSpawns()
    print("[GV-ESP] Initial scan complete")
end)
...
lp.CharacterAdded:Connect(function()
    ...
    pcall(function()
        gvCleanupEntities()
        if gvEsp.on then gvScanForSpawns() end
        if gvEsp.on then gvShowAll() end
    end)
end)
...
-- ============================================================
-- GROUNDBULB & VINE ESP UI - FIXED WITH WORKING TOGGLES
-- ============================================================

catSection:Toggle({ 
    Title="Groundbulb & Vine ESP", 
    Type="Checkbox", 
    Flag="gvEspOn", 
    Default=gvEsp.on,
    Callback=function(on)
        pcall(function()
            if on then 
                gvStart() 
                ui:Notify({Title="GV ESP", Content="Enabled", Duration=2, Icon="check"})
            else 
                gvStop()
                ui:Notify({Title="GV ESP", Content="Disabled", Duration=2, Icon="x"})
            end
        end)
    end 
})
...
-- GV ESP Visual Settings
visSection:ColorPicker({
    Title = "GV Highlight Color",
    Flag = "gvColor",
    Default = Color3.fromRGB(170, 0, 255),
    Callback = function(color)
        pcall(function()
            gvConfig.HighlightColor = color
            for _, data in pairs(gvEsp.trackedEntities) do
...
visSection:Slider({
    Title = "GV Fill Transparency",
    Flag = "gvFillTransparency",
    Step = 0.05,
...
visSection:Button({ 
    Title="Refresh GV ESP", 
    Callback=function()
        pcall(function()
            gvCleanupEntities()
            gvScanForSpawns()
            if gvEsp.on then gvShowAll() end
            ui:Notify({Title="GV ESP", Content="Refreshed", Duration=2, Icon="refresh-cw"})
        end)
    end 
})

visSection:Button({ 
    Title="Clear GV Highlights", 
    Callback=function()
        pcall(function()
            for model, data in pairs(gvEsp.trackedEntities) do
            ...
            ui:Notify({Title="GV ESP", Content="Cleared all highlights", Duration=2, Icon="trash-2"})
        end)
    end 
})
...
local function elliotAimAt(tgt)
    ...
    if elliotAimType == "HRP Aimbot" or elliotAimType == "Camera + Character" then
        if elliotHRP and elliotHum then
            if not arManager.locks["elliot"] then
                arLock("elliot", elliotHum)
            end
...
local function elliotStartAimbot()
    ...
    elliotConnection = svc.Run.RenderStepped:Connect(function()
        if not elliotEnabled or not elliotHum or not elliotHRP then
            if arManager.locks["elliot"] and elliotHum then
                arUnlock("elliot", elliotHum)
            end
            return
        end
...
-- ============================================================
-- GUEST 1337 - CORE BLOCK TAB (Truncated for length)
-- ============================================================

-- [Guest 1337 tabs continue here...]
...
function applyConfigFromFlags()
    pcall(function()
        ...
        -- GV ESP Flags
        if win.Flags.gvEspOn ~= nil then
            if win.Flags.gvEspOn then gvStart() else gvStop() end
        end
        if win.Flags.gvShowGroundbulb ~= nil then 
            gvEsp.showGroundbulb = win.Flags.gvShowGroundbulb
            if gvEsp.on then gvShowAll() end
        end
        if win.Flags.gvShowVine ~= nil then 
            gvEsp.showVine = win.Flags.gvShowVine
            if gvEsp.on then gvShowAll() end
        end
        if win.Flags.gvFillTransparency ~= nil then 
            gvConfig.FillTransparency = win.Flags.gvFillTransparency
            for _, data in pairs(gvEsp.trackedEntities) do
                if data.Highlight then
                    data.Highlight.FillTransparency = win.Flags.gvFillTransparency
                end
            end
        end
...
print("[GV-ESP] Groundbulb & Vine ESP integrated with Humanoid filter")
