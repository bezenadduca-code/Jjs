-- FORSAKEN ESP + SAKIWARE MERGED
-- Original ESP by ESP | SAKIWARE maintained by mitsuki | original by v1pr/glov
print("Forsaken + SAKIWARE loaded")
pcall(function()
...
local sakiConfig    = ConfigManager:CreateConfig("forsaken_saki")
...
local function removeESP()
    if CurrentESP and CurrentESP.Parent then CurrentESP:Destroy() end
    if CurrentBillboard and CurrentBillboard.Parent then CurrentBillboard:Destroy() end
    CurrentESP       = nil
    CurrentBillboard = nil
end

local function makeESP(obj)
    if CurrentESP then return end
...
local function scan(v)
    if v:IsA("MeshPart") then makeESP(v) end
end
...
workspace.DescendantAdded:Connect(function(v)
    task.wait(0.1)
    if Enabled and not CurrentESP then scan(v) end
end)
...
do
    ...
    function ChatLogger.createUI()
        ...
        pcall(function()
            ...
            chatScreenGui.Name = "ChatLoggerScreen"; chatScreenGui.ResetOnSpawn = false
            chatScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; chatScreenGui.DisplayOrder = 10; chatScreenGui.Parent = pg
            chatWindow = Instance.new("Frame"); chatWindow.Name = "ChatLoggerUI"
            chatWindow.Size = UDim2.new(0, 340, 0, 180); chatWindow.Position = UDim2.new(0, 80, 0, 10)
            chatWindow.AnchorPoint = Vector2.new(0, 0); chatWindow.BackgroundColor3 = Color3.fromRGB(30, 30, 35)
            chatWindow.BackgroundTransparency = 0.15; chatWindow.BorderSizePixel = 0
            chatWindow.ClipsDescendants = true; chatWindow.Parent = chatScreenGui
            ...
            titleBar.Size = UDim2.new(1, 0, 0, 32); titleBar.BackgroundColor3 = Color3.fromRGB(40, 40, 45)
            ...
            local titleText = Instance.new("TextLabel"); titleText.Size = UDim2.new(1, -100, 1, 0)
            titleText.Position = UDim2.new(0, 12, 0, 0); titleText.BackgroundTransparency = 1
            titleText.Text = "💬 Chat Logger"; titleText.TextColor3 = Color3.fromRGB(220, 220, 220)
            titleText.TextSize = 13; titleText.TextXAlignment = Enum.TextXAlignment.Left
            titleText.Font = Enum.Font.GothamBold; titleText.Parent = titleBar
            local closeBtn = Instance.new("TextButton"); closeBtn.Size = UDim2.new(0, 32, 1, 0)
            closeBtn.Position = UDim2.new(1, -32, 0, 0); closeBtn.BackgroundTransparency = 1
            closeBtn.Text = "X"; closeBtn.TextColor3 = Color3.fromRGB(180, 180, 180)
            ...
            chatScrollFrame = Instance.new("ScrollingFrame"); chatScrollFrame.Name = "ChatScroller"
            chatScrollFrame.Size = UDim2.new(1, 0, 1, -70); chatScrollFrame.Position = UDim2.new(0, 0, 0, 32)
            chatScrollFrame.BackgroundTransparency = 1; chatScrollFrame.BorderSizePixel = 0
            chatScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 0); chatScrollFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
            chatScrollFrame.ScrollBarThickness = 6; chatScrollFrame.ScrollBarImageColor3 = Color3.fromRGB(90, 90, 90)
            chatScrollFrame.VerticalScrollBarPosition = Enum.VerticalScrollBarPosition.Right; chatScrollFrame.Parent = chatWindow
            chatContainer = Instance.new("UIListLayout"); chatContainer.Parent = chatScrollFrame
            chatContainer.SortOrder = Enum.SortOrder.LayoutOrder; chatContainer.Padding = UDim.new(0, 2)
            local pad = Instance.new("UIPadding"); pad.PaddingLeft = UDim.new(0, 8); pad.PaddingRight = UDim.new(0, 8)
            pad.PaddingTop = UDim.new(0, 5); pad.PaddingBottom = UDim.new(0, 5); pad.Parent = chatScrollFrame
            local inputFrame = Instance.new("Frame"); inputFrame.Name = "InputFrame"
            inputFrame.Size = UDim2.new(1, 0, 0, 38); inputFrame.Position = UDim2.new(0, 0, 1, -38)
            inputFrame.BackgroundColor3 = Color3.fromRGB(35, 35, 40); inputFrame.BackgroundTransparency = 0.2
            inputFrame.BorderSizePixel = 0; inputFrame.Parent = chatWindow
            local ic = Instance.new("UICorner"); ic.CornerRadius = UDim.new(0, 6); ic.Parent = inputFrame
            chatInput = Instance.new("TextBox"); chatInput.Name = "ChatInput"
            chatInput.Size = UDim2.new(1, -56, 1, -8); chatInput.Position = UDim2.new(0, 8, 0, 4)
            chatInput.BackgroundColor3 = Color3.fromRGB(25, 25, 30); chatInput.BackgroundTransparency = 0.3
            chatInput.Text = ""; chatInput.TextColor3 = Color3.fromRGB(255, 255, 255); chatInput.TextSize = 13
            ...
            chatInput.PlaceholderColor3 = Color3.fromRGB(150, 150, 150); chatInput.ClearTextOnFocus = false; chatInput.Parent = inputFrame
            local ic2 = Instance.new("UICorner"); ic2.CornerRadius = UDim.new(0, 4); ic2.Parent = chatInput
            local sendBtn = Instance.new("TextButton"); sendBtn.Size = UDim2.new(0, 40, 1, -8)
            sendBtn.Position = UDim2.new(1, -48, 0, 4); sendBtn.BackgroundColor3 = Color3.fromRGB(80, 80, 90)
    ...
    function ChatLogger.addMessage(sender, message, messageType)
        pcall(function()
            ...
            msgOrder = msgOrder + 1
            local nameColor = COLORS.Player; local textColor = Color3.fromRGB(240, 240, 240); local prefix = ""
            ...
            local msgFrame = Instance.new("Frame"); msgFrame.Name = "Msg_"..msgOrder; msgFrame.LayoutOrder = msgOrder
            ...
            local line = Instance.new("TextLabel"); line.Size = UDim2.new(1, 0, 0, 0); line.AutomaticSize = Enum.AutomaticSize.Y
            line.BackgroundTransparency = 1; line.TextColor3 = textColor; line.TextSize = 12
            line.TextXAlignment = Enum.TextXAlignment.Left; line.TextWrapped = true; line.RichText = true; line.Font = Enum.Font.Gotham
            line.Text = string.format('<font color="#%02x%02x%02x">[%s]</font> <font color="#%02x%02x%02x"><b>%s%s</b></font>: %s',
                ...
                math.floor(nameColor.R*255), math.floor(nameColor.G*255), math.floor(nameColor.B*255), prefix, sender, message)
    ...
    function ChatLogger.clear()
        pcall(function()
            ...
            msgOrder = 0; ChatLogger.addMessage("System", "Chat log cleared!", "system")
        end)
    end
            ...
            local function disableEffects(parent)
                ...
                for _, obj in ipairs(parent:GetDescendants()) do
                    ...
                    if obj:IsA("Sound") and obj.Name and (obj.Name:lower():find("taph") or obj.Name:lower():find("blind") or obj.Name:lower():find("static")) then
                        pcall(function() obj.Volume = 0 end); pcall(function() obj:Stop() end)
                    end
                end
            end
            disableEffects(svc.WS.CurrentCamera); disableEffects(svc.WS); disableEffects(Lighting)
    ...
    while next(pool) do
        ...
        for k, node in pairs(pool) do
            if flowNeighbour(cur.row, cur.col, node.row, node.col) then
                table.insert(ordered, { row = node.row, col = node.col }); pool[k] = nil; cur = node; moved = true; break
            end
        end
...
local function flowSolve(puzzle)
    ...
    for _, ci in ipairs(indices) do
        ...
        for _, node in ipairs(ordered) do
            table.insert(puzzle.paths[ci], { row = node.row, col = node.col }); puzzle:updateGui(); task.wait(flow.nodeDelay)
        end
...
do
    ...
    if fgModule then
        ...
        else warn("[sakiware] FlowGame: failed to require FlowGame module") end
    else warn("[sakiware] FlowGame: Modules.Minigames.FlowGameManager.FlowGame not found") end
end
...
local function aimRefreshTargets()
    ...
    for _,model in ipairs(sf:GetChildren()) do
        if model~=lp.Character and model:IsA("Model") then
            ...
            if h and r and h.Health>0 and r:IsDescendantOf(workspace) then table.insert(aim.cache, { model=model, humanoid=h, root=r }) end
        end
    end
end
...
task.spawn(function()
    ...
    if not remote then warn("[sakiware] Aimbot: could not find RemoteEvent"); return end
...
local function coolkidGetInputDir()
    local cf=svc.WS.CurrentCamera.CFrame; local camFwd=Vector3.new(cf.LookVector.X,0,cf.LookVector.Z); local camRight=Vector3.new(cf.RightVector.X,0,cf.RightVector.Z)
    local x,z=0,0
...
svc.Run.RenderStepped:Connect(function(dt)
    ...
    local dir=coolkidGetInputDir(); local lv=hrp:FindFirstChildWhichIsA("LinearVelocity")
...
local noliVoidRushOn=false; local noliOverrideActive=false; local noliOrigWalkSpeed=nil; local noliConn=nil
local function noliStop()
    if not noliOverrideActive then return end; noliOverrideActive=false
    ...
    noliOrigWalkSpeed=nil; if noliConn then noliConn:Disconnect(); noliConn=nil end
end
local function noliStart()
    if noliOverrideActive then return end; noliOverrideActive=true
    noliConn=svc.Run.RenderStepped:Connect(function()
        local char=lp.Character; local hum=char and char:FindFirstChild("Humanoid"); local root=char and char:FindFirstChild("HumanoidRootPart")
...
local esp = {
    killers=false, survivors=false, generators=false, items=false, buildings=false,
...
local function espItemColor(name)
    local n=name:lower()
    if n:find("medkit") or n:find("bloxycola") then return Color3.fromRGB(0,255,255) end
    return Color3.fromRGB(0,255,255)
end
local function espItemHeld(obj)
    for _,plr in ipairs(svc.Players:GetPlayers()) do
        local ch=plr.Character; if ch and obj:IsDescendantOf(ch) then return true end
        local bp=plr:FindFirstChildOfClass("Backpack"); if bp and obj:IsDescendantOf(bp) then return true end
    end
    return false
end
...
espAttach = function(obj, tag, color, isChar)
    ...
    pcall(function() local h=obj:FindFirstChild(tag); if h then h:Destroy() end; local b=obj:FindFirstChild(tag.."_bb"); if b then b:Destroy() end end)
...
local function espDoKillers(on)   if not esp.killerFolder   then return end; for _,k in ipairs(esp.killerFolder:GetChildren())   do if k:IsA("Model") then if on then espAttach(k,"esp_k",Color3.fromRGB(255,80,80),true)   else espDetach(k,"esp_k")   end end end end
local function espDoSurvivors(on) if not esp.survivorFolder then return end; for _,s in ipairs(esp.survivorFolder:GetChildren()) do if s:IsA("Model") then if on then espAttach(s,"esp_s",Color3.fromRGB(50,255,50),true)  else espDetach(s,"esp_s")  end end end end
local function espDoGenerators(on) local map=getMapContent(); if not map then return end; for _,obj in ipairs(map:GetChildren()) do if obj.Name=="Generator" then if on then espAttach(obj,"esp_g",Color3.fromRGB(255,105,180),false) else espDetach(obj,"esp_g") end end end end
...
local function espDoBuildings(on) local ig=getIngame(); if not ig then return end; for _,obj in ipairs(ig:GetChildren()) do if obj.Name=="BuildermanSentry" or obj.Name=="SubspaceTripmine" or obj.Name=="BuildermanDispenser" then if on then espAttach(obj,"esp_b",Color3.fromRGB(255,80,0),false) else espDetach(obj,"esp_b") end end end end
...
local function addHighlight(obj, color, tag, label, offset)
    ...
    local conn; conn=obj.AncestryChanged:Connect(function()
        ...
        local bb=obj:FindFirstChild(tag.."_BB"); if bb then bb:Destroy() end; tracked[tag][obj]=nil
    end)
end
...
local function addPuddleHighlight(part, color, tag, label)
    ...
    local conn; conn=part.AncestryChanged:Connect(function()
        ...
        pcall(function() sizeConn:Disconnect() end); pcall(function() hl:Destroy() end); pcall(function() holder:Destroy() end); tracked[tag][part]=nil
    end)
end
...
local function musicGetSound()
    local t=svc.WS:FindFirstChild("Themes"); if not t then return nil end
...
do
    ...
    local elliotEnabled=false; local elliotConnection=nil; local elliotAutoRotBak=nil
    local elliotPredDist=5; local elliotVelThresh=16; local elliotAimType="Camera + Character"
    local elliotThrowDur=0.5; local elliotIsThrowing=false; local elliotThrowTS=0
    local elliotRequireAnim=true; local elliotShowArc=false; local elliotArcFolder=nil
    local elliotArcParts={}; local elliotArcSegs=50; local elliotThrowForce=80
    local elliotUpComp=0.5; local elliotGravity=196.2; local elliotHum,elliotHRP=nil,nil
    local elliotCamera=svc.WS.CurrentCamera; local elliotTargetMode="Low HP"
    local function elliotSetupChar(char) elliotHum=char:WaitForChild("Humanoid"); elliotHRP=char:WaitForChild("HumanoidRootPart") end
    if lp.Character then elliotSetupChar(lp.Character) end
    lp.CharacterAdded:Connect(function(c) elliotSetupChar(c) end)
    task.spawn(function()
        local ok,re=pcall(function() return svc.RS:WaitForChild("Modules",5):WaitForChild("Network",5):WaitForChild("Network",5):WaitForChild("RemoteEvent",5) end)
        if ok and re then
            local oldNC; oldNC=hookmetamethod(game,"__namecall",function(self,...) local method=getnamecallmethod(); local args={...}
                if method=="FireServer" and self==re then
                    if args[1]=="UseActorAbility" and args[2] and args[2][1] then
                        ...
                        if ok2 and bs and string.find(bs,"ThrowPizza") then elliotIsThrowing=true; elliotThrowTS=tick() end
                    end
                end
                return oldNC(self,...)
            end)
    ...
    local function elliotClearArc() for _,p in ipairs(elliotArcParts) do if p and p.Parent then p:Destroy() end end; elliotArcParts={} end
    local function elliotCreateArcFolder() if elliotArcFolder then elliotArcFolder:Destroy() end; elliotArcFolder=Instance.new("Folder"); elliotArcFolder.Name="ElliotArc"; elliotArcFolder.Parent=svc.WS end
    local function elliotFindTarget()
        ...
        for _,s in ipairs(sf:GetChildren()) do
            if s~=lp.Character then
                local h=s:FindFirstChildOfClass("Humanoid"); local r=s:FindFirstChild("HumanoidRootPart")
    ...
    local function elliotAimAt(tgt)
        ...
        local vel=tgt.AssemblyLinearVelocity; local pos=tgt.Position
        ...
        if elliotAimType=="HRP Aimbot" or elliotAimType=="Camera + Character" then
            if elliotHRP then
                ...
                elliotHum.AutoRotate=false; elliotHRP.AssemblyAngularVelocity=Vector3.new(0,0,0)
                local dir=(predPos-elliotHRP.Position); local flat=Vector3.new(dir.X,0,dir.Z).Unit
                local tCF=CFrame.new(elliotHRP.Position,elliotHRP.Position+flat); local cur=elliotHRP.CFrame
                local nCF=cur:Lerp(tCF,0.35); elliotHRP.CFrame=CFrame.new(cur.Position)*nCF.Rotation
            end
        end
    ...
    local function elliotArcCalc(startPos,lookVec)
        local dir=(lookVec+Vector3.new(0,elliotUpComp,0)).Unit; local iv=dir*elliotThrowForce
        local maxT=3; local pts={}; local step=maxT/elliotArcSegs; local last=startPos
        local rp=RaycastParams.new(); rp.FilterType=Enum.RaycastFilterType.Exclude; rp.FilterDescendantsInstances={lp.Character,elliotArcFolder}
        for i=0,elliotArcSegs do
            local t=i*step; local pos=startPos+iv*t+Vector3.new(0,-0.5*elliotGravity*t*t,0)
            if i>0 then local d=pos-last; local dm=d.Magnitude; if dm>0 then local res=svc.WS:Raycast(last,d.Unit*dm,rp); if res then table.insert(pts,res.Position); break end end end
            if pos.Y<-100 then break end; table.insert(pts,pos); last=pos
        end
        return pts
    end
    ...
    local function elliotUpdateArc()
        ...
        local now=tick(); if now-_elliotLastArcUpdate<0.1 then return end; _elliotLastArcUpdate=now
        local char=lp.Character; local lArm=char and (char:FindFirstChild("Left Arm") or char:FindFirstChild("LeftHand") or char:FindFirstChild("LeftLowerArm"))
        ...
        elliotClearArc(); if not elliotArcFolder then elliotCreateArcFolder() end
        for i,p in ipairs(pts) do
            local part=Instance.new("Part"); part.Name="ArcSeg"..i; part.Size=Vector3.new(0.25,0.25,0.25); part.Position=p; part.Anchored=true; part.CanCollide=false; part.Material=Enum.Material.Neon; part.Shape=Enum.PartType.Ball
    ...
    sec_014:Toggle({ Title="Show Pizza Arc",            Flag="elliotShowArc",  Default=false, Callback=function(v) elliotShowArc=v; if v then elliotCreateArcFolder() else elliotClearArc(); if elliotArcFolder then elliotArcFolder:Destroy(); elliotArcFolder=nil end end end, Type="Checkbox" })
    ...
    sec_014:Toggle({ Title="Enable Elliot Aimbot", Flag="elliotEnabled", Default=false, Callback=function(v)
        ...
        if v then
            elliotConnection=svc.Run.RenderStepped:Connect(function()
                ...
                if not shouldAim then if elliotAutoRotBak~=nil then elliotHum.AutoRotate=elliotAutoRotBak; elliotAutoRotBak=nil end; return end
                local tgt=elliotFindTarget()
                if not tgt then if elliotAutoRotBak~=nil then elliotHum.AutoRotate=elliotAutoRotBak; elliotAutoRotBak=nil end; return end
                elliotAimAt(tgt)
            end)
        else
...
do
    ...
    local chanceAimEnabled=false; local chancePredMode="Velocity"; local chancePredValue=0.5
    local chanceAimBehavior="Normal"; local chanceSpinDur=0.5; local chanceMsgOnAim=false
    local chanceMsgText=""; local chanceAntiBait=true; local chanceSmoothSpeed=14
    local chanceHeightAim=true; local chanceHoldToAim=true; local chanceAimKey=Enum.KeyCode.Q
    local chanceHoldingKey=false; local chanceAiming=false; local chanceStartTime=0
    local chanceAimDuration=1.7
    local chanceKillerSpeeds={Slasher={walk=9,run=28},c00lkidd={walk=7.75,run=28},JohnDoe={walk=9,run=27.25},["1x1x1x1"]={walk=8.5,run=27},Noli={walk=7.5,run=27.5},Guest666={walk=9,run=27},Nosferatu={walk=7.25,run=27.5},Doombringer={walk=8,run=27},JaneDoe={walk=9,run=27},Builderman={walk=8.5,run=27.5},Dusekkar={walk=8,run=27.5}}
    ...
    local function chanceGetMotion(hrp)
        ...
        data.lastPos=pos; data.lastTime=now; data.accel=acc; data.velocity=vel; return vel,acc
    end
    local chancePingSamples={}; local _chanceLastPingTime=0; local _chanceLastPingVal=0.1
    local function chanceGetPing()
        local now=tick(); if now-_chanceLastPingTime<1 then return _chanceLastPingVal end; _chanceLastPingTime=now
        ...
        local s=0; for _,v in ipairs(chancePingSamples) do s=s+v end; _chanceLastPingVal=s/#chancePingSamples; return _chanceLastPingVal
    end
    local function chanceGetNearest()
        if not chanceHRP then return end; local folder=getTeamFolder("Killers"); if not folder then return end
    ...
    local function chancePredict(hrp)
        ...
        local ping=chanceGetPing(); local dist=chanceHRP and (hrp.Position-chanceHRP.Position).Magnitude or 0; local ds=dist*0.003; local lead
        ...
        local ac=accel*lead*lead*0.5; if ac.Magnitude>speed*0.4 then ac=ac.Unit*(speed*0.4) end
    ...
    svc.Run.RenderStepped:Connect(function()
        ...
        if not chanceBodyGyro or not chanceBodyGyro.Parent then
            chanceBodyGyro=Instance.new("BodyGyro"); chanceBodyGyro.MaxTorque=Vector3.new(0,math.huge,0); chanceBodyGyro.P=10000; chanceBodyGyro.D=500; chanceBodyGyro.Parent=chanceHRP
        end
...
do
    ...
    local function atStopManager()
        if atDescendantAddedConn and atDescendantAddedConn.Connected then atDescendantAddedConn:Disconnect() end; atDescendantAddedConn=nil
        for _,cleanup in pairs(atActiveMonitors) do if type(cleanup)=="function" then pcall(cleanup) end end; atActiveMonitors={}
    end
    sec_022:Toggle({ Title="Auto Trick", Flag="veeeAutoTrick", Default=false, Callback=function(on)
        atEnabled=on
        if on then if not atBehaviorFolder then local ok,f=pcall(atGetBehaviorFolder); if ok and f then atBehaviorFolder=f end end; atStartManager() else atStopManager() end
    end, Type="Checkbox" })
end
...
do
    ...
    local sk8_chargeAnimIds={"117058860640843"}; local sk8_DASH_SPEED=60; local sk8_controlEnabled=true; local sk8_controlActive=false
    local sk8_overrideConn=nil; local sk8_savedHumState={}
    ...
    svc.Run.RenderStepped:Connect(function()
        ...
        if sk8_detectChargeAnim() then if not sk8_controlActive then sk8_startOverride() end else if sk8_controlActive then sk8_stopOverride() end end
    end)
    ...
    sec_023:Toggle({ Title="Enable SK8 Control", Default=sk8_controlEnabled, Flag="sk8ControlEnabled", Callback=function(on) sk8_controlEnabled=on; if not on and sk8_controlActive then sk8_stopOverride() end end, Type="Checkbox" })
end
...
do
    ...
    sec_vee_spray:Toggle({ Title="Enable Fast Spray", Type="Checkbox", Flag="veeeFastSpray", Default=false, Callback=function(on) vee_fastSpray=on; if not on then sprayPhase=0; sprayBaseCF=nil end end })
end
...
do
    local jd_Run=svc.Run; local jd_RS=svc.RS; local jd_lp=lp; local jd_Camera=svc.WS.CurrentCamera
    local jd_RemoteEvent=nil; local jd_NetworkRF=nil
    pcall(function() jd_RemoteEvent=jd_RS:WaitForChild("Modules",10):WaitForChild("Network",10):WaitForChild("Network",10):WaitForChild("RemoteEvent",10) end)
    pcall(function() jd_NetworkRF=jd_RS:WaitForChild("Modules",10):WaitForChild("Network",10):WaitForChild("Network",10):WaitForChild("RemoteFunction",10) end)
    local jd_enabled=false; local jd_aimbotOn=false; local jd_patched=false; local jd_crystalCB=nil; local jd_unloaded=false
    local jd_AIM_OFFSET=-0.3; local jd_PREDICTION=0.6; local jd_HOLD_DURATION=0.9
    local jd_axeEnabled=false; local jd_AXE_LOCK_DURATION=1.7; local jd_axeLocked=false
    ...
    local function jd_getKillerVelocity(hrp)
        ...
        local vel=(pos-data.lastPos)/dt; data.lastPos=pos; data.lastTime=now; data.velocity=vel; return vel
    end
    ...
    local function jd_holdCrystal()
        ...
        local b=buffer.create(8); buffer.writeu32(b,0,2); buffer.writef32(b,4,svc.WS.DistributedGameTime)
        jd_RemoteEvent:FireServer(jd_lp.Name.."CrystalInput",{b})
    end
    local function jd_axeDoLock()
        if jd_axeLocked then return end; local char=jd_lp.Character; local myHRP=char and char:FindFirstChild("HumanoidRootPart"); local hum=char and char:FindFirstChildOfClass("Humanoid")
        if not myHRP or not hum then return end; local killer=jd_getNearestKiller(myHRP.Position); local killerHRP=killer and killer:FindFirstChild("HumanoidRootPart"); if not killerHRP then return end
        jd_axeLocked=true; local savedAutoRotate=hum.AutoRotate; hum.AutoRotate=false
        local deadline=tick()+jd_AXE_LOCK_DURATION
        local conn; conn=svc.Run.RenderStepped:Connect(function()
            if tick()>=deadline or not jd_axeEnabled then conn:Disconnect(); pcall(function() hum.AutoRotate=savedAutoRotate end); jd_axeLocked=false; return end
            if not myHRP.Parent or not killerHRP.Parent then conn:Disconnect(); pcall(function() hum.AutoRotate=savedAutoRotate end); jd_axeLocked=false; return end
            local dir=Vector3.new(killerHRP.Position.X-myHRP.Position.X,0,killerHRP.Position.Z-myHRP.Position.Z)
            if dir.Magnitude>0 then myHRP.CFrame=CFrame.new(myHRP.Position,myHRP.Position+dir.Unit) end
        end)
    end
    local function jd_buildCamCF(myHRP,killerHRP,v0,g)
        local hum=myHRP.Parent and myHRP.Parent:FindFirstChildOfClass("Humanoid"); local hipH=hum and hum.HipHeight or 1.35
        local v238=(hipH+myHRP.Size.Y/2)/2; local spawnPos=myHRP.CFrame.Position+Vector3.new(0,v238,0)
        local vel=jd_getKillerVelocity(killerHRP); local predicted=killerHRP.Position+vel*jd_PREDICTION
        local target=predicted+Vector3.new(0,jd_AIM_OFFSET,0); local delta=target-spawnPos
        local flatV=Vector3.new(delta.X,0,delta.Z); local dx=flatV.Magnitude; local dy=delta.Y
        if dx<0.01 then local d=dy>=0 and Vector3.new(0,1,0) or Vector3.new(0,-1,0); return CFrame.new(jd_Camera.CFrame.Position,jd_Camera.CFrame.Position+d) end
        local flatDir=flatV.Unit; local v2=v0*v0; local disc=v2*v2-g*(g*dx*dx+2*dy*v2)
        local theta=disc<0 and math.atan2(dy,dx) or math.atan2(v2-math.sqrt(disc),g*dx); local T=math.tan(theta); local denom=3+T
        local alpha=math.abs(denom)<0.0001 and -math.pi/2 or math.atan2(3*T-1,denom)
        local yawCF=CFrame.new(jd_Camera.CFrame.Position,jd_Camera.CFrame.Position+flatDir); return yawCF*CFrame.Angles(alpha,0,0)
    end
    ...
    local function jd_applyPatch(actor)
        ...
        if type(getcallbackvalue)=="function" then pcall(function() jd_crystalCB=getcallbackvalue(jd_NetworkRF,"OnClientInvoke") end) end
            ...
            local ok,result=pcall(function(...)
                if jd_enabled and jd_aimbotOn then
                    ...
                    if myHRP then
                        ...
                        if killerHRP then
                            if reqName=="GetMousePosition" then local vel=jd_getKillerVelocity(killerHRP); local predPos=killerHRP.Position+vel*jd_PREDICTION+Vector3.new(0,jd_AIM_OFFSET,0); return predPos end
                            if reqName=="GetCameraCF" then local ok2,cf=pcall(jd_buildCamCF,myHRP,killerHRP,250,40); if ok2 and cf then return cf end end
                        end
                    end
                end
                if jd_crystalCB then return jd_crystalCB(reqName,...) end
            end,...)
            if not ok then if jd_crystalCB then return jd_crystalCB(reqName,...) end end; return result
        end
        jd_patched=true
    end
        ...
        local oldNC; oldNC=hookmetamethod(game,"__namecall",function(self,...)
            ...
            if method=="FireServer" and self==re then
                ...
                if jd_enabled and eventName==(jd_lp.Name.."CrystalInput") then
                    if not jd_holdActive then
                        ...
                        task.spawn(function()
                            ...
                            while tick()<deadline and jd_enabled and not jd_unloaded do jd_holdCrystal(); task.wait(1/30) end
                            jd_holdActive=false
                        end)
                    end
                end
                if jd_axeEnabled and eventName=="UseActorAbility" and args[2] and args[2][1] then
                    local ok2,bs=pcall(function() return buffer.tostring(args[2][1]) end)
                    if ok2 and bs and bs:find("Axe") then task.spawn(jd_axeDoLock) end
                end
            end
            return oldNC(self,...)
        end)
    ...
    task.spawn(function()
        ...
        while not jd_unloaded do
            task.wait(0.5); local cur=jd_getLocalActor()
            if cur~=lastActor then
                if lastActor~=nil then jd_patched=false; jd_crystalCB=nil; jd_killerMotionData={} end
                lastActor=cur; if cur and jd_enabled then jd_applyPatch(cur) end
            end
        end
    end)
    local sec_024=tabJaneDoe:Section({ Title="Crystal Auto-Fire", Opened=true })
    sec_024:Toggle({ Title="Enable Jane Doe Aimbot", Flag="jdEnabled", Default=false, Callback=function(on) jd_enabled=on; local actor=jd_getLocalActor(); if on and not jd_patched and actor then jd_applyPatch(actor) end end, Type="Checkbox" })
    sec_024:Toggle({ Title="Aimbot (Silent Aim)",    Flag="jdSilentAim", Default=false, Callback=function(on) jd_aimbotOn=on; local actor=jd_getLocalActor(); if on and not jd_patched and actor then jd_applyPatch(actor) end end, Type="Checkbox" })
    ...
    sec_025:Toggle({ Title="Enable Axe Lock-On", Flag="jdAxeEnabled", Default=false, Callback=function(on) jd_axeEnabled=on end, Type="Checkbox" })
    ...
    sec_026:Button({ Title="Unload Jane Doe", Callback=function()
        if jd_unloaded then return end; jd_unloaded=true; jd_enabled=false; jd_aimbotOn=false; jd_axeEnabled=false; pcall(jd_removePatch)
    end })
end
...
do
    ...
    local plasma_enabled=false; local plasma_aimOffset=0.0; local plasma_prediction=0.12
    local plasma_oldCB=nil; local plasma_rf=nil; local plasma_motionData={}
    local function plasmaGetNearestKiller()
        local char=lp.Character; if not char then return nil end; local myHRP=char:FindFirstChild("HumanoidRootPart"); if not myHRP then return nil end
        local pf=svc.WS:FindFirstChild("Players"); local kf=pf and pf:FindFirstChild("Killers"); if not kf then return nil end
        local best,bestDist=nil,math.huge
        for _,model in ipairs(kf:GetChildren()) do
            if model~=char then local hrp=model:FindFirstChild("HumanoidRootPart"); local hum=model:FindFirstChildOfClass("Humanoid")
                if hrp and hum and hum.Health>0 then local d=(hrp.Position-myHRP.Position).Magnitude; if d<bestDist then bestDist=d; best=hrp end end
            end
        end
        return best
    end
    local function plasmaGetVelocity(hrp)
        local now=tick(); local pos=hrp.Position; local data=plasma_motionData[hrp]
        if not data then plasma_motionData[hrp]={lastPos=pos,lastTime=now,vel=Vector3.zero}; return Vector3.zero end
        local dt=now-data.lastTime
        if dt>0 then data.vel=(pos-data.lastPos)/dt; data.lastPos=pos; data.lastTime=now end; return data.vel
    end
    local function plasmaPatchRF()
        if plasma_rf then return end
        local ok,rf=pcall(function() return svc.RS:WaitForChild("Modules",10):WaitForChild("Network",10):WaitForChild("Network",10):WaitForChild("RemoteFunction",10) end)
        if not ok or not rf then warn("[SAKIWARE] Dusekkar: RemoteFunction not found"); return end
        plasma_rf=rf; plasma_oldCB=getcallbackvalue(rf,"OnClientInvoke")
        rf.OnClientInvoke=function(reqName,data,...)
            if reqName=="GetMousePosition" and plasma_enabled then
                ...
                if hrp then local vel=plasmaGetVelocity(hrp); local predPos=hrp.Position+vel*plasma_prediction+Vector3.new(0,plasma_aimOffset,0); return predPos end
            end
            if plasma_oldCB then return plasma_oldCB(reqName,data,...) end
        end
    end
    local function plasmaUnpatch() if plasma_rf and plasma_oldCB then plasma_rf.OnClientInvoke=plasma_oldCB end; plasma_rf=nil; plasma_oldCB=nil end
    task.spawn(plasmaPatchRF)
    lp.CharacterAdded:Connect(function() plasma_motionData={} end)
    sec_027:Toggle({ Title="Enable PlasmaBeam Aim", Default=plasma_enabled, Type="Checkbox", Flag="plasmaEnabled", Callback=function(on) plasma_enabled=on end })
    sec_027:Slider({ Title="Prediction (s)",    Flag="plasmaPrediction", Value={Min=0.0,Max=0.5,Default=plasma_prediction}, Step=0.01, Callback=function(v) plasma_prediction=v end })
    sec_027:Slider({ Title="Aim Height Offset", Flag="plasmaAimOffset",  Value={Min=-5.0,Max=5.0,Default=plasma_aimOffset}, Step=0.1,  Callback=function(v) plasma_aimOffset=v   end })
    local sec_028=tabDusekkar:Section({ Title="Control", Opened=true })
    sec_028:Button({ Title="Unload PlasmaBeam Hook", Callback=function() plasma_enabled=false; plasmaUnpatch() end })
end
...
do
    ...
    local nova_enabled=false; local nova_aimOffset=0.0; local nova_prediction=0.12
    local nova_oldCB=nil; local nova_rf=nil; local nova_motionData={}
    local function novaGetVelocity(hrp)
        local now=tick(); local pos=hrp.Position; local data=nova_motionData[hrp]
        if not data then nova_motionData[hrp]={lastPos=pos,lastTime=now,vel=Vector3.zero}; return Vector3.zero end
        local dt=now-data.lastTime
        if dt>0 then data.vel=(pos-data.lastPos)/dt; data.lastPos=pos; data.lastTime=now end; return data.vel
    end
    local function novaGetNearestSurvivor()
        local char=lp.Character; if not char then return nil end; local myHRP=char:FindFirstChild("HumanoidRootPart"); if not myHRP then return nil end
        local pf=svc.WS:FindFirstChild("Players"); local sf=pf and pf:FindFirstChild("Survivors"); if not sf then return nil end
        local best,bestDist=nil,math.huge
        for _,model in ipairs(sf:GetChildren()) do
            if model~=char then local hrp=model:FindFirstChild("HumanoidRootPart"); local hum=model:FindFirstChildOfClass("Humanoid")
                if hrp and hum and hum.Health>0 then local d=(hrp.Position-myHRP.Position).Magnitude; if d<bestDist then bestDist=d; best=hrp end end
    ...
    local function novaPatch()
        ...
        nova_rf=rf; nova_oldCB=getcallbackvalue(rf,"OnClientInvoke")
        rf.OnClientInvoke=function(reqName,data,...)
            if reqName=="GetMousePosition" and nova_enabled then
                local hrp=novaGetNearestSurvivor()
                if hrp then local vel=novaGetVelocity(hrp); local predPos=hrp.Position+vel*nova_prediction+Vector3.new(0,nova_aimOffset,0); return predPos end
            end
            if nova_oldCB then return nova_oldCB(reqName,data,...) end
        end
    end
    local function novaUnpatch() if nova_rf and nova_oldCB then nova_rf.OnClientInvoke=nova_oldCB end; nova_rf=nil; nova_oldCB=nil end
    task.spawn(novaPatch)
    lp.CharacterAdded:Connect(function() nova_motionData={} end)
    sec_029:Toggle({ Title="Enable Nova Aim", Default=nova_enabled, Type="Checkbox", Flag="novaEnabled", Callback=function(on) nova_enabled=on end })
    sec_029:Slider({ Title="Prediction (s)",    Flag="novaPrediction", Value={Min=0.0,Max=0.5,Default=nova_prediction}, Step=0.01, Callback=function(v) nova_prediction=v end })
    sec_029:Slider({ Title="Aim Height Offset", Flag="novaAimOffset",  Value={Min=-5.0,Max=5.0,Default=nova_aimOffset}, Step=0.1,  Callback=function(v) nova_aimOffset=v   end })
    local sec_029b=tabNoli:Section({ Title="Control", Opened=true })
    sec_029b:Button({ Title="Unload Nova Hook", Callback=function() nova_enabled=false; novaUnpatch() end })
end
...
secAIMain:Paragraph({ Title="What this does", Content="Pathfinds to the nearest survivor using PathfindingService. Killer-only — switch to killer before enabling." })

local ai_enabled=false; local ai_resetOnDeath=true; local ai_thread=nil
local PathfindingService=game:GetService("PathfindingService")
local aiCfg={ slashRange=15, slashCooldown=2, pathInterval=0.6, predScale=0.08 }
local aiState={ target=nil, lastSlash=0, lastPath=0, waypoints={}, wpIndex=1, moveConn=nil, lastMovePos=nil, stuckCheck={pos=nil,time=0} }

local function aiGetSurvivorsFolder() local p=svc.WS:FindFirstChild("Players"); return p and p:FindFirstChild("Survivors") end
local function aiPredictPosition(hrp,t) local vel=hrp.AssemblyLinearVelocity or Vector3.zero; return hrp.Position+vel*t end
local function aiGetNearest()
    local char=lp.Character; if not char then return end; local myHRP=char:FindFirstChild("HumanoidRootPart"); if not myHRP then return end
    local sf=aiGetSurvivorsFolder(); if not sf then return end; local best,dist=nil,math.huge
    for _,m in ipairs(sf:GetChildren()) do
        local h=m:FindFirstChildOfClass("Humanoid"); local r=m:FindFirstChild("HumanoidRootPart")
        if h and r and h.Health>0 then local d=(aiPredictPosition(r,aiCfg.predScale)-myHRP.Position).Magnitude; if d<dist then dist=d; best={model=m,humanoid=h,root=r} end end
    end
    return best
end
local function aiGetMyKillerName()
    local ch=lp.Character; if not ch then return nil end; local kf=getTeamFolder("Killers"); if not kf then return nil end
    if not ch:IsDescendantOf(kf) then return nil end; return ch.Name
end
local function aiFireSlash()
    local re=hbGetRemote(); if not re then return end; local killerName=aiGetMyKillerName() or ""; local abilityName,bufStr
    if killerName=="Noli" then abilityName="Stab"; bufStr="\x03\x04\x00\x00\x00Stab"
    elseif killerName=="c00lkidd" then abilityName="Punch"; bufStr="\x03\x05\x00\x00\x00Punch"
    else abilityName="Slash"; bufStr="\x03\x05\x00\x00\x00Slash" end
    local ok,buf=pcall(function() return buffer.fromstring(bufStr) end)
    if ok and buf then pcall(function() re:FireServer("UseActorAbility",{[1]=buf}) end) end
...
local function aiStartMove()
    ...
    aiState.moveConn=svc.Run.Heartbeat:Connect(function()
        if not ai_enabled then return end; local char=lp.Character; if not char then return end
        local hrp=char:FindFirstChild("HumanoidRootPart"); local hum=char:FindFirstChildOfClass("Humanoid")
        if not hrp or not hum or hum.Health<=0 then return end; if not aiState.target then return end
        local targetHRP=aiState.target.root; if not targetHRP then return end
        local directDist=(targetHRP.Position-hrp.Position).Magnitude
        if directDist<=8 then aiState.waypoints={}; aiState.wpIndex=1; aiState.lastMovePos=nil; hum:MoveTo(targetHRP.Position); return end
        if aiState.wpIndex<=#aiState.waypoints then
            local wp=aiState.waypoints[aiState.wpIndex]; if (hrp.Position-wp.Position).Magnitude<5 then aiState.wpIndex+=1 end; hum:MoveTo(wp.Position); return
        end
        aiState.lastMovePos=aiState.lastMovePos or hrp.Position
        local targetPos=aiPredictPosition(targetHRP,aiCfg.predScale); aiState.lastMovePos=aiState.lastMovePos:Lerp(targetPos,0.15); hum:MoveTo(aiState.lastMovePos)
    end)
end
local function aiKillerLoop()
    aiStartMove()
    while ai_enabled do
        task.wait(0.1); local char=lp.Character; if not char then continue end
        local hrp=char:FindFirstChild("HumanoidRootPart"); if not hrp then continue end
        local hum=char:FindFirstChildOfClass("Humanoid")
        if not hum or hum.Health<=0 then if ai_resetOnDeath then pcall(function() hum:ChangeState(Enum.HumanoidStateType.Dead) end) end; task.wait(3); continue end
        local now=tick()
        if not aiState.stuckCheck.pos then aiState.stuckCheck.pos=hrp.Position; aiState.stuckCheck.time=now
        elseif now-aiState.stuckCheck.time>=1.5 then
            local moved=(hrp.Position-aiState.stuckCheck.pos).Magnitude
            if moved<2 and aiState.target and aiState.target.root and (aiState.target.root.Position-hrp.Position).Magnitude>8 then aiState.waypoints={}; aiState.wpIndex=1; aiState.lastPath=0 end
            aiState.stuckCheck.pos=hrp.Position; aiState.stuckCheck.time=now
        end
        if not aiState.target or not aiState.target.root or aiState.target.humanoid.Health<=0 then aiState.target=aiGetNearest(); aiState.waypoints={}; aiState.wpIndex=1 end
        if not aiState.target then continue end
        local targetHRP=aiState.target.root
        local dist=(targetHRP.Position-hrp.Position).Magnitude
        if dist<=aiCfg.slashRange then if tick()-aiState.lastSlash>aiCfg.slashCooldown then aiFireSlash(); aiState.lastSlash=tick() end end
        if tick()-aiState.lastPath>aiCfg.pathInterval then
            aiState.lastPath=tick()
            local path=PathfindingService:CreatePath({AgentRadius=2,AgentHeight=5,AgentCanJump=true})
            local ok=pcall(function() path:ComputeAsync(hrp.Position,aiPredictPosition(targetHRP,aiCfg.predScale)) end)
            if ok and path.Status==Enum.PathStatus.Success then aiState.waypoints=path:GetWaypoints(); aiState.wpIndex=1 else aiState.waypoints={} end
        end
    end
    if aiState.moveConn then aiState.moveConn:Disconnect(); aiState.moveConn=nil end
end

secAIMain:Toggle({ Title="Enable Killer AI Farm", Type="Checkbox", Flag="aiKillerEnabled", Default=ai_enabled, Callback=function(on)
    ai_enabled=on
    if on then
        if ai_thread then task.cancel(ai_thread) end; aiState.target=nil; aiState.waypoints={}; aiState.wpIndex=1; aiState.lastMovePos=nil
        ai_thread=task.spawn(aiKillerLoop)
    else
        ai_enabled=false; if ai_thread then task.cancel(ai_thread); ai_thread=nil end
        if aiState.moveConn then aiState.moveConn:Disconnect(); aiState.moveConn=nil end
        aiState.target=nil; aiState.waypoints={}; aiState.wpIndex=1
        local char=lp.Character; if char then local hum=char:FindFirstChildOfClass("Humanoid"); if hum then hum:MoveTo(char.HumanoidRootPart.Position) end end
    end
end })
secAIMain:Toggle({ Title="Auto Reset on Death", Type="Checkbox", Flag="aiResetOnDeath", Default=ai_resetOnDeath, Callback=function(on) ai_resetOnDeath=on end })
...
local combatS={autoBlockOn=false,blockType="Block",detectionRange=18,blockDelay=0,doubleBlock=true,antiBait=false,abMissChance=0,autoPunchOn=false,hdtEnabled=false,hdtSpeed=12,hdtDelay=0,hdtMissChance=0,killerCircles=false,facingCheck=true,facingVisual=false,facingVisRadius=3,aimPunchActive=false,punchPrediction=2.3,aimPunchDuration=0.5}

local TRIGGER_SOUNDS={["140242176732868"]=true,["136323728355613"]=true,["115026634746636"]=true,["84116622032112"]=true,["108907358619313"]=true,["127793641088496"]=true,["86174610237192"]=true,["95079963655241"]=true,["101199185291628"]=true,["119942598489800"]=true,["84307400688050"]=true,["105200830849301"]=true,["75330693422988"]=true,["82221759983649"]=true,["81702359653578"]=true,["85853080745515"]=true,["108610718831698"]=true,["112395455254818"]=true,["109431876587852"]=true,["12222216"]=true,["79980897195554"]=true,["119583605486352"]=true,["71834552297085"]=true,["116581754553533"]=true,["86833981571073"]=true,["110372418055226"]=true,["105840448036441"]=true,["86494585504534"]=true,["80516583309685"]=true,["131406927389838"]=true,["89004992452376"]=true,["117231507259853"]=true,["101698569375359"]=true,["101553872555606"]=true,["140412278320643"]=true,["106300477136129"]=true,["117173212095661"]=true,["104910828105172"]=true,["140194172008986"]=true,["85544168523099"]=true,["114506382930939"]=true,["99829427721752"]=true,["120059928759346"]=true,["104625283622511"]=true,["105316545074913"]=true,["126131675979001"]=true,["82336352305186"]=true,["93366464803829"]=true,["84069821282466"]=true,["128856426573270"]=true,["121954639447247"]=true,["128195973631079"]=true,["124903763333174"]=true,["94317217837143"]=true,["98111231282218"]=true,["119089145505438"]=true,["136728245733659"]=true,["107444859834748"]=true,["76959687420003"]=true,["72425554233832"]=true,["96594507550917"]=true,["139996647355899"]=true,["107345261604889"]=true,["127557531826290"]=true,["108651070773439"]=true,["74842815979546"]=true,["124397369810639"]=true,["76467993976301"]=true,["118493324723683"]=true,["78298577002481"]=true,["116527305931161"]=true,["5148302439"]=true,["98675142200448"]=true,["128367348686124"]=true,["71805956520207"]=true,["125213046326879"]=true,["103684883268194"]=true,["109246041199659"]=true,["80540530406270"]=true,["139523195429581"]=true,["105204810054381"]=true,["114742322778642"]=true,["116468089135195"]=true,["112809109188560"]=true,["106727013904874"]=true}
local BLOCK_ANIMS={["72722244508749"]=true,["96959123077498"]=true,["95802026624883"]=true,["100926346851492"]=true,["120748030255574"]=true}
local BAIT_KILLERS={"John Doe","Slasher","c00lkidd","Jason","1x1x1x1","Noli","Sixer","Nosferatu"}
local STRICT_FACING_DOT=0.70; local _cachedAnimator=nil

local function combatIsFacing(myRoot,targetRoot,killerName)
    if not combatS.facingCheck then return true end; if not myRoot or not targetRoot then return false end
    local diff=myRoot.Position-targetRoot.Position; if diff.Magnitude<0.01 then return true end
    local dir=diff.Unit; local dot=targetRoot.CFrame.LookVector:Dot(dir); local bait=false
    if killerName then for _,n in ipairs(BAIT_KILLERS) do if killerName:find(n) then bait=true; break end end end
    if bait then
        local vel=Vector3.zero; pcall(function() vel=targetRoot.AssemblyLinearVelocity end); if vel.Magnitude<0.01 then pcall(function() vel=targetRoot.Velocity end) end
        local side=math.abs(vel:Dot(targetRoot.CFrame.RightVector)); if side>3 then return false end
...
local function combatGetNearestKiller()
    local char=lp.Character; if not char then return nil end; local myRoot=char:FindFirstChild("HumanoidRootPart"); if not myRoot then return nil end
    local kf=combatGetKillersFolder(); if not kf then return nil end; local best,bestD=nil,math.huge
    for _,k in pairs(kf:GetChildren()) do local hrp=k:FindFirstChild("HumanoidRootPart"); if hrp then local d=(hrp.Position-myRoot.Position).Magnitude; if d<bestD then best,bestD=k,d end end end
    return best
end
...
local function combatFireAbility(abilityType)
    local rem=hbGetRemote(); if not rem then return end; local buf
    if abilityType=="Block" then buf=buffer.fromstring("\x03\x05\x00\x00\x00Block")
    elseif abilityType=="Punch" then buf=buffer.fromstring("\x03\x05\x00\x00\x00Punch")
    elseif abilityType=="Charge" then buf=buffer.fromstring("\x03\x06\x00\x00\x00Charge")
    elseif abilityType=="Clone" then buf=buffer.fromstring("\x03\x05\x00\x00\x00Clone")
    else buf=buffer.fromstring("\x03\x05\x00\x00\x00Block") end
    pcall(function() rem:FireServer("UseActorAbility",{[1]=buf}) end); pcall(function() rem:FireServer(abilityType) end)
end
local combatGetKillerHRP
local combatTrackedPunchAnimations={["87259391926321"]=true,["140703210927645"]=true,["136007065400978"]=true,["129843313690921"]=true,["86709774283672"]=true,["108807732150251"]=true,["138040001965654"]=true,["86096387000557"]=true,["81905101227053"]=true,["127777649118195"]=true,["99100240941590"]=true,["92831180929659"]=true,["112081768119093"]=true,["117587689359268"]=true,["91830732867282"]=true,["91730605416216"]=true,["100184164753080"]=true}
local combatPunchAiming=false; local combatPunchLastTriggerTime=0; local combatOriginalAutoRotate=nil; local combatOriginalHRPCFrame=nil; local combatAimConnection=nil
local function combatSetupAimPunch(char)
    ...
    local hum=char:FindFirstChild("Humanoid"); local anim=hum and hum:FindFirstChildOfClass("Animator")
    ...
    combatAimConnection=anim.AnimationPlayed:Connect(function(track)
        ...
        if combatS.aimPunchActive and combatTrackedPunchAnimations[animId] then
            ...
            if h and r and not combatPunchAiming then combatOriginalAutoRotate=h.AutoRotate; combatOriginalHRPCFrame=r.CFrame end
            combatPunchLastTriggerTime=tick(); combatPunchAiming=true
        end
    end)
end
svc.Run.RenderStepped:Connect(function()
    if not combatS.aimPunchActive then
        if combatPunchAiming then
            combatPunchAiming=false; local char=lp.Character; local hum=char and char:FindFirstChild("Humanoid"); local hrp=char and char:FindFirstChild("HumanoidRootPart")
            if hrp and combatOriginalHRPCFrame~=nil then hrp.CFrame=CFrame.new(hrp.Position)*combatOriginalHRPCFrame.Rotation; hrp.AssemblyAngularVelocity=Vector3.zero; combatOriginalHRPCFrame=nil end
            if hum then hum.AutoRotate=combatOriginalAutoRotate~=nil and combatOriginalAutoRotate or true; combatOriginalAutoRotate=nil end
        end
        return
    end
    if not combatPunchAiming then return end
    local elapsed=tick()-combatPunchLastTriggerTime; local char=lp.Character; local hum=char and char:FindFirstChild("Humanoid"); local hrp=char and char:FindFirstChild("HumanoidRootPart")
    ...
    hum.AutoRotate=false; hrp.AssemblyAngularVelocity=Vector3.zero
    local kf=svc.WS:FindFirstChild("Players") and svc.WS.Players:FindFirstChild("Killers")
    if kf then
        local bestDist,targetHRP=math.huge,nil
        for _,killer in ipairs(kf:GetChildren()) do local khrp=killer:FindFirstChild("HumanoidRootPart"); if khrp then local d=(khrp.Position-hrp.Position).Magnitude; if d<bestDist then bestDist=d; targetHRP=khrp end end end
        if targetHRP then
            local vel=targetHRP.AssemblyLinearVelocity or Vector3.zero
            local predictPos=vel.Magnitude>0.5 and (targetHRP.Position+vel*(combatS.punchPrediction/60)) or targetHRP.Position
            local dir=(predictPos-hrp.Position)*Vector3.new(1,0,1); if dir.Magnitude>0.01 then hrp.CFrame=CFrame.new(hrp.Position,hrp.Position+dir.Unit) end
        end
    end
end)

local combatSoundHooks={}; local combatSoundBlockedUntil={}; local combatLastBlockTime=0; local BLOCK_CD=0.1
local function combatExtractSoundId(sound) if not sound then return nil end; return tostring(sound.SoundId):match("%d+") end
local function combatTryBlockFromSound(sound,preId)
    if not combatS.autoBlockOn then return end; if not sound or not sound:IsA("Sound") then return end
    local id=preId or combatExtractSoundId(sound); if not id or not TRIGGER_SOUNDS[id] then return end
    local now=tick(); if now-combatLastBlockTime<BLOCK_CD then return end
    ...
    local soundPart
    if sound.Parent and sound.Parent:IsA("BasePart") then soundPart=sound.Parent
    elseif sound.Parent and sound.Parent:IsA("Attachment") and sound.Parent.Parent and sound.Parent.Parent:IsA("BasePart") then soundPart=sound.Parent.Parent
    else soundPart=sound.Parent and sound.Parent:FindFirstChildWhichIsA("BasePart",true) end
    local killerModel=nil
    if soundPart then
        local model=soundPart:FindFirstAncestorOfClass("Model")
        if model and model:FindFirstChildOfClass("Humanoid") then local kf=combatGetKillersFolder(); if kf and model:IsDescendantOf(kf) then killerModel=model end end
    end
    if not killerModel then killerModel=combatGetNearestKiller() end; if not killerModel then return end
    local hrp=killerModel:FindFirstChild("HumanoidRootPart"); if not hrp then return end
    local dist=(hrp.Position-myRoot.Position).Magnitude; if dist>combatS.detectionRange then return end
    if not combatIsFacing(myRoot,hrp,killerModel.Name) then return end
    if combatS.antiBait then
        local vel=Vector3.zero; pcall(function() vel=hrp.AssemblyLinearVelocity end); if vel.Magnitude<0.1 then pcall(function() vel=hrp.Velocity end) end
        ...
        if dist>6 then local sideSpeed=math.abs(vel:Dot(hrp.CFrame.RightVector)); if sideSpeed>6 and vel:Dot(toUs.Unit)<0 then return end end
    end
    if combatRollMiss(combatS.abMissChance) then return end
    combatLastBlockTime=now; combatSoundBlockedUntil[sound]=now+0.3
...
local function combatHookSound(sound)
    ...
    local preId=combatExtractSoundId(sound); if not preId then return end
    local playedConn=sound.Played:Connect(function() if combatS.autoBlockOn then task.spawn(combatTryBlockFromSound,sound,preId) end end)
    local propConn=sound:GetPropertyChangedSignal("IsPlaying"):Connect(function() if sound.IsPlaying and combatS.autoBlockOn then task.spawn(combatTryBlockFromSound,sound,preId) end end)
    local destroyConn; destroyConn=sound.Destroying:Connect(function() pcall(function() playedConn:Disconnect(); propConn:Disconnect(); destroyConn:Disconnect() end); combatSoundHooks[sound]=nil; combatSoundBlockedUntil[sound]=nil end)
...
local function combatHookExistingSounds()
    ...
    for _,killer in pairs(kf:GetChildren()) do for _,desc in pairs(killer:GetDescendants()) do if desc:IsA("Sound") then pcall(combatHookSound,desc) end end end
end
local function combatSetupSoundWatcher()
    task.spawn(function()
        local playersFolder=svc.WS:FindFirstChild("Players"); if not playersFolder then playersFolder=svc.WS:WaitForChild("Players",30) end; if not playersFolder then return end
        local kf=playersFolder:FindFirstChild("Killers"); if not kf then kf=playersFolder:WaitForChild("Killers",30) end; if not kf then return end
        combatHookExistingSounds()
        kf.DescendantAdded:Connect(function(desc) if desc:IsA("Sound") then pcall(combatHookSound,desc) end end)
        kf.ChildAdded:Connect(function(killer) task.wait(0.1); for _,desc in pairs(killer:GetDescendants()) do if desc:IsA("Sound") then pcall(combatHookSound,desc) end end end)
    end)
end

local combatHDTLastTime=0; local HDT_CD=0.5; local _combatHDTDebounce=false
combatGetKillerHRP=function(killerModel)
    if not killerModel then return nil end; local hrp=killerModel:FindFirstChild("HumanoidRootPart"); if hrp then return hrp end
    if killerModel.PrimaryPart then return killerModel.PrimaryPart end; return killerModel:FindFirstChildWhichIsA("BasePart",true)
end
local function combatHDTBeginDrag(killerModel)
    if _combatHDTDebounce then return end; if not killerModel or not killerModel.Parent then return end
    local char=lp.Character; if not char then return end; local hrp=char:FindFirstChild("HumanoidRootPart"); local hum=char:FindFirstChildOfClass("Humanoid")
    if not hrp or not hum then return end; local targetHRP=combatGetKillerHRP(killerModel); if not targetHRP then return end
    if combatRollMiss(combatS.hdtMissChance) then return end
    _combatHDTDebounce=true
    local oldWalk=hum.WalkSpeed; local oldJump=hum.JumpPower; local oldPlatformStand=hum.PlatformStand
    hum.WalkSpeed=0; hum.JumpPower=0; hum.PlatformStand=false
    local bv=Instance.new("BodyVelocity"); bv.MaxForce=Vector3.new(1e5,0,1e5); bv.Velocity=Vector3.new(0,0,0); bv.Parent=hrp
    local conn; conn=svc.Run.Heartbeat:Connect(function(dt)
        if not _combatHDTDebounce then conn:Disconnect(); if bv and bv.Parent then pcall(function() bv:Destroy() end) end; hum.WalkSpeed=oldWalk; hum.JumpPower=oldJump; hum.PlatformStand=oldPlatformStand; return end
        if not(char and char.Parent) or not(killerModel and killerModel.Parent) then _combatHDTDebounce=false; return end
        targetHRP=combatGetKillerHRP(killerModel); if not targetHRP then _combatHDTDebounce=false; return end
        local toTarget=targetHRP.Position-hrp.Position; local horiz=Vector3.new(toTarget.X,0,toTarget.Z)
        if horiz.Magnitude>0.01 then local dir=horiz.Unit; bv.Velocity=Vector3.new(dir.X*combatS.hdtSpeed,bv.Velocity.Y,dir.Z*combatS.hdtSpeed) else bv.Velocity=Vector3.new(0,bv.Velocity.Y,0) end
        if toTarget.Magnitude<=2.0 then _combatHDTDebounce=false end
    end)
    task.spawn(function()
        hum.AutoRotate=false; local sw=tick()
        while tick()-sw<0.4 do pcall(function() local nk=combatGetNearestKiller(); if nk and hrp and hrp.Parent then local tHRP2=combatGetKillerHRP(nk); if tHRP2 then hrp.CFrame=CFrame.lookAt(hrp.Position,tHRP2.Position) end end end); task.wait() end
        hum.AutoRotate=true
    end)
    task.delay(0.4,function() if _combatHDTDebounce then _combatHDTDebounce=false end end)
end
local function combatOnBlockAnim(track)
    pcall(function()
        local id=tostring(track.Animation and track.Animation.AnimationId or ""):match("%d+"); if not id or not BLOCK_ANIMS[id] then return end
        if combatS.hdtEnabled and not _combatHDTDebounce then
            local now=tick(); if now-combatHDTLastTime>=HDT_CD then
                combatHDTLastTime=now; local nearest=combatGetNearestKiller()
                if nearest then task.spawn(function() if combatS.hdtDelay>0 then task.wait(combatS.hdtDelay) end; combatHDTBeginDrag(nearest) end) end
            end
        end
        if combatS.autoPunchOn then task.delay(0.12,function() combatFireAbility("Punch") end) end
    end)
end
local combatCircles={}; local combatFacingVisuals={}
local function combatUpdateCircles()
    local kf=combatGetKillersFolder(); if not kf then return end
    for _,k in pairs(kf:GetChildren()) do
        local hrp=k:FindFirstChild("HumanoidRootPart"); if hrp then
            if combatS.killerCircles then
                if not combatCircles[k] then pcall(function() local c=Instance.new("CylinderHandleAdornment"); c.Name="CombatCircle"; c.Adornee=hrp; c.Color3=Color3.fromRGB(255,140,170); c.AlwaysOnTop=true; c.ZIndex=1; c.Transparency=0.6; c.Radius=combatS.detectionRange; c.Height=0.12; c.CFrame=CFrame.new(0,-(hrp.Size.Y/2+0.05),0)*CFrame.Angles(math.rad(90),0,0); c.Parent=hrp; combatCircles[k]=c end)
                else combatCircles[k].Radius=combatS.detectionRange end
            else if combatCircles[k] then combatCircles[k]:Destroy(); combatCircles[k]=nil end end
        end
    end
    for k,c in pairs(combatCircles) do if not k.Parent or not k:FindFirstChild("HumanoidRootPart") then pcall(function() c:Destroy() end); combatCircles[k]=nil end end
end
local function combatUpdateFacing()
    local kf=combatGetKillersFolder(); if not kf then return end; local myRoot=lp.Character and lp.Character:FindFirstChild("HumanoidRootPart")
    for _,k in pairs(kf:GetChildren()) do
        local hrp=k:FindFirstChild("HumanoidRootPart"); if hrp then
            if combatS.facingVisual then
                if not combatFacingVisuals[k] then pcall(function() local v=Instance.new("CylinderHandleAdornment"); v.Name="FacingVis"; v.Adornee=hrp; v.AlwaysOnTop=true; v.ZIndex=2; v.Radius=combatS.facingVisRadius; v.Height=0.08; v.CFrame=CFrame.new(0,-(hrp.Size.Y/2+0.04),-combatS.facingVisRadius)*CFrame.Angles(math.rad(90),0,0); v.Color3=Color3.fromRGB(120,255,120); v.Transparency=0.3; v.Parent=hrp; combatFacingVisuals[k]=v end) end
                local vis=combatFacingVisuals[k]; if vis and vis.Parent then
                    vis.Radius=combatS.facingVisRadius; vis.CFrame=CFrame.new(0,-(hrp.Size.Y/2+0.04),-combatS.facingVisRadius)*CFrame.Angles(math.rad(90),0,0)
                    ...
                    elseif inRange then vis.Color3=Color3.fromRGB(255,120,120); vis.Transparency=0.4
                    else vis.Color3=Color3.fromRGB(255,255,120); vis.Transparency=0.7 end
                end
            else if combatFacingVisuals[k] then combatFacingVisuals[k]:Destroy(); combatFacingVisuals[k]=nil end end
        end
    end
end
...
local function combatStartLoops()
    ...
    combatSoundTickConn=svc.Run.Heartbeat:Connect(function()
        if not combatS.autoBlockOn then return end; local now=tick()
        for sound,t in pairs(combatSoundBlockedUntil) do if now>t then combatSoundBlockedUntil[sound]=nil end end
    end)
...
local function combatStopLoops()
    ...
    combatCircles={}; combatFacingVisuals={}
end
local function combatRefreshAnimator()
    local c=lp.Character; if not c then _cachedAnimator=nil; return end
    local h=c:FindFirstChildOfClass("Humanoid"); _cachedAnimator=h and h:FindFirstChildOfClass("Animator") or nil
    if _cachedAnimator then _cachedAnimator.AnimationPlayed:Connect(combatOnBlockAnim) end
end
lp.CharacterAdded:Connect(function(char) task.wait(0.6); combatRefreshAnimator(); combatSetupAimPunch(char); if combatS.autoBlockOn then combatSetupSoundWatcher() end; if combatS.autoBlockOn or combatS.killerCircles or combatS.facingVisual then combatStartLoops() end end)
if lp.Character then task.spawn(function() task.wait(1); combatRefreshAnimator(); combatSetupAimPunch(lp.Character); if combatS.autoBlockOn then combatSetupSoundWatcher() end; if combatS.autoBlockOn or combatS.killerCircles or combatS.facingVisual then combatStartLoops() end end) end
...
local _g1337AbToggle = sec_015:Toggle({ Title="Auto Block (Audio)", Flag="combatAutoBlock", Default=combatS.autoBlockOn, Callback=function(on)
    ...
    if on then combatSetupSoundWatcher(); combatStartLoops() else combatStopLoops() end
end, Type="Checkbox" })
...
sec_018:Toggle({ Title="Detection Circles", Flag="combatCircles", Default=combatS.killerCircles, Callback=function(on) combatS.killerCircles=on; if on then combatStartLoops() else combatUpdateCircles() end end, Type="Checkbox" })
...
sec_018:Toggle({ Title="Facing Visual",     Flag="combatFacingVis",   Default=combatS.facingVisual, Callback=function(on) combatS.facingVisual=on; if on then combatStartLoops() end end, Type="Checkbox" })
sec_018:Slider({ Title="Facing Visual Size",Flag="combatFacingSize",  Value={Min=1,Max=10,Default=combatS.facingVisRadius}, Step=0.5, Callback=function(v)
    ...
    for _,vis in pairs(combatFacingVisuals) do if vis and vis.Parent then vis.Radius=v; local adornee=vis.Adornee; if adornee then vis.CFrame=CFrame.new(0,-(adornee.Size.Y/2+0.04),-v)*CFrame.Angles(math.rad(90),0,0) end end end
end })

local sec_019=tabGuest1337:Section({ Title="Aim Punch Lock", Opened=true })
sec_019:Toggle({ Title="Aim Punch", Flag="combatAimPunch", Default=combatS.aimPunchActive, Callback=function(on)
    combatS.aimPunchActive=on; if on and lp.Character then combatSetupAimPunch(lp.Character) end
    if not on and combatAimConnection then combatAimConnection:Disconnect(); combatAimConnection=nil end
end, Type="Checkbox" })
sec_019:Slider({ Title="Punch Prediction", Flag="combatPunchPred", Step=0.1, Value={Min=0,Max=10,Default=combatS.punchPrediction}, Callback=function(v) combatS.punchPrediction=v end })
sec_019:Slider({ Title="Aim Duration (s)", Flag="combatAimDur",   Step=0.05, Value={Min=0.1,Max=2.0,Default=combatS.aimPunchDuration}, Callback=function(v) combatS.aimPunchDuration=v end })

------------------------------------------------------------------------
-- SETTINGS TAB: Interface / Chat Logger / Anti-Taph / AB Dot
------------------------------------------------------------------------
local _abDotGui=nil; local _abDotBtn=nil; local _abDotDragging,_abDotDragStart,_abDotDragPos=false,nil,nil; local _settingsAbToggle=nil

local function abDotSetState(on)
    if _abDotBtn then
        _abDotBtn.BackgroundColor3=on and Color3.fromRGB(40,200,80) or Color3.fromRGB(200,40,40)
        _abDotBtn.Text=on and "AB" or "ab"; _abDotBtn.TextColor3=Color3.fromRGB(255,255,255)
    end
...
local function abDotCreate()
    ...
    local pg=lp:FindFirstChildOfClass("PlayerGui"); if not pg then return end
    _abDotGui=Instance.new("ScreenGui"); _abDotGui.Name="ABDotGui"; _abDotGui.ResetOnSpawn=false; _abDotGui.DisplayOrder=20; _abDotGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; _abDotGui.Parent=pg
    _abDotBtn=Instance.new("TextButton"); _abDotBtn.Size=UDim2.new(0,52,0,52); _abDotBtn.Position=UDim2.new(1,-64,0.5,-26); _abDotBtn.AnchorPoint=Vector2.new(0,0); _abDotBtn.BackgroundColor3=Color3.fromRGB(200,40,40); _abDotBtn.BackgroundTransparency=0; _abDotBtn.BorderSizePixel=0; _abDotBtn.Text="ab"; _abDotBtn.TextColor3=Color3.fromRGB(255,255,255); _abDotBtn.TextSize=13; _abDotBtn.Font=Enum.Font.GothamBold; _abDotBtn.AutoButtonColor=false; _abDotBtn.Parent=_abDotGui
    ...
    _abDotBtn.MouseButton1Click:Connect(function()
        if _abDotDragging then return end; combatS.autoBlockOn=not combatS.autoBlockOn; abDotSetState(combatS.autoBlockOn)
        if combatS.autoBlockOn then combatSetupSoundWatcher(); combatStartLoops() else combatStopLoops() end
    end)
    local _abMouseHeld=false
    _abDotBtn.InputBegan:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then _abMouseHeld=true; _abDotDragging=false; _abDotDragStart=inp.Position; _abDotDragPos=_abDotBtn.Position end end)
    _abDotBtn.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then _abMouseHeld=false; _abDotDragStart=nil; task.delay(0.05,function() _abDotDragging=false end) end end)
    svc.Input.InputChanged:Connect(function(inp)
        if not _abMouseHeld or not _abDotDragStart then return end; if inp.UserInputType~=Enum.UserInputType.MouseMovement and inp.UserInputType~=Enum.UserInputType.Touch then return end
        local delta=inp.Position-_abDotDragStart; if delta.Magnitude>6 then _abDotDragging=true end
        if _abDotDragging and _abDotBtn and _abDotBtn.Parent then _abDotBtn.Position=UDim2.new(_abDotDragPos.X.Scale,_abDotDragPos.X.Offset+delta.X,_abDotDragPos.Y.Scale,_abDotDragPos.Y.Offset+delta.Y) end
    end)
...
_settingsAbToggle=secInterface:Toggle({ Title="Auto Block", Type="Checkbox", Flag="settingsAutoBlock", Default=combatS.autoBlockOn,
    Callback=function(on) combatS.autoBlockOn=on; abDotSetState(on); if on then combatSetupSoundWatcher(); combatStartLoops(); abDotCreate() else combatStopLoops() end end
})
...
secChatLogger:Toggle({ Title="Enable Chat Logger", Type="Checkbox", Flag="chatLogEnabled", Default=chatLogEnabled,
    Callback=function(on) chatLogEnabled=on; if on then ChatLogger.setup() else ChatLogger.cleanup() end end })
...
secAntiTaph:Toggle({ Title="Remove Blindness / Effects", Type="Checkbox", Flag="antiTaphEnabled", Default=antiTaphEnabled,
    Callback=function(on) antiTaphEnabled=on; if on then AntiTaph.apply() else AntiTaph.remove() end end })

------------------------------------------------------------------------
-- TAB: INTERFACE
------------------------------------------------------------------------
local tabInterface = Window:Tab({ Title = "Interface", Icon = "layout-dashboard" })
local sec_030 = tabInterface:Section({ Title = "UI Functions", Opened = true })
sec_030:Button({ Title="Close UI", Callback=function() local ok=pcall(function() Window:Destroy() end); if not ok then pcall(function() Window:Close() end) end end })

print("Forsaken + SAKIWARE ready")
...
task.spawn(function()
    while true do task.wait(2); sakiConfig:Save() end
end)
