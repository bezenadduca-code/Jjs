...
-- forward-declare so Settings toggle can reference them; assigned later after combat section
local combatSetupSoundWatcher, combatStartLoops, combatStopLoops
-- AB dot and Settings toggle are defined after the Guest 1337 combat section (below)
-- so that combatSetupSoundWatcher/combatStartLoops/combatStopLoops are already assigned.
local _abDotGui = nil
local _abDotBtn = nil
local _abDotDragging, _abDotDragStart, _abDotDragPos = false, nil, nil
local _settingsAbToggle = nil
local _g1337AbToggle = nil
local abDotSetState   -- forward-declare; defined after combat section
local abDotCreate     -- forward-declare; defined after combat section
...
-- UI Elements
_g1337AbToggle = sec_015:Toggle({ Title = "Auto Block (Audio)", Flag = "combatAutoBlock", Default = combatS.autoBlockOn, Callback=function(on) 
...
------------------------------------------------------------------------
-- AB indicator dot + Settings toggle (defined here so combat functions are in scope)
------------------------------------------------------------------------
-- ── AB indicator dot ────────────────────────────────────────────────
abDotSetState = function(on)
    if _abDotBtn then
