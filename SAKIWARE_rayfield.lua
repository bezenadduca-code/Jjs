...
local win = Rayfield:CreateWindow({
    Name             = "SAKIWARE",
    LoadingTitle     = "SAKIWARE",
    LoadingSubtitle  = "maintained by mitsuki",
    ConfigurationSaving = {
        Enabled    = true,
        FolderName = "SAKIWARE",
        FileName   = "sakiware",
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabSettings = win:CreateTab("Settings", 0)
...
local secChatLogger = tabSettings:CreateSection("Chat Logger")
secChatLogger:CreateToggle({ Name = "Enable Chat Logger", Flag = "chatLogEnabled", CurrentValue = false,
    Callback = function(on)
        chatLogEnabled = on
        if on then ChatLogger.setup() else ChatLogger.cleanup() end
    end
})
secChatLogger:CreateButton({ Name = "Show / Hide Window", Callback = function() ChatLogger.toggle() end })
secChatLogger:CreateButton({ Name = "Clear Log",          Callback = function() ChatLogger.clear()  end })

pcall(function()
    local secFullbright = tabSettings:CreateSection("Fullbright")
    ...
    secFullbright:CreateToggle({ Name = "Fullbright", Flag = "fullbrightOn", CurrentValue = false,
        Callback = function(on)
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabGlobal  = win:CreateTab("Global", 0)
...
secStamina:CreateToggle({ Name = "Custom Stamina", Flag = "stamOn", CurrentValue = stam.on,
...
secStamina:CreateToggle({ Name = "Infinite Stamina", Flag = "stamNoLoss", CurrentValue = stam.noLoss,
    Callback = function(on)
...
-- Speed Hack
------------------------------------------------------------------------
local secSpeed = tabGlobal:CreateSection("Speed Hack")
...
secSpeed:CreateToggle({ Name = "Custom Sprint Speed", Flag="speedOn", CurrentValue = speedHack.on,
    Callback=function(on) speedHack.on=on; speedHack.lastApplied=0; if on then speedStart() else speedStop() end end })
secSpeed:CreateInput({
...
secSpeed:CreateButton({ Name = "Reset to Default", Callback=function() speedStop() end })
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabGen     = win:CreateTab("Generator", 0)
...
secGenAuto:CreateToggle({ Name = "Auto Solve", Flag = "flowOn", CurrentValue = flow.on, Callback = function(on) flow.on = on end
})
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabKiller = win:CreateTab("Killer", 0)
...
secAimbot:CreateToggle({ Name = "Enable Aimbot", Flag="aimOn",       CurrentValue = aim.on,       Callback=function(on) aim.on=on;       if not on then aimUnlock() end end })
...
secABS:CreateToggle({ Name = "Enable Anti-Backstab", Flag="absOn", CurrentValue = abs.on, Callback=function(on) abs.on=on; if on then absStart() else absStop() end end })
secABS:CreateSlider({
...
secKillerAbilities:CreateToggle({ Name = "c00lkidd — Dash Turn", Flag="coolkidWSOOn",  CurrentValue = coolkidWSOOn,  Callback=function(on) coolkidWSOOn=on;    end })
secKillerAbilities:CreateToggle({ Name = "Noli — Void Rush Control", Flag="noliVoidRushOn",CurrentValue = noliVoidRushOn,Callback=function(on) noliVoidRushOn=on; if not on then noliStop() end end })
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabVisual = win:CreateTab("Visual", 0)
...
secESP:CreateToggle({ Name = "Killers", Flag="espKillers",    CurrentValue = esp.killers,    Callback=function(on) esp.killers=on;    task.spawn(function() espDoKillers(on)    end) end })
secESP:CreateToggle({ Name = "Survivors", Flag="espSurvivors",  CurrentValue = esp.survivors,  Callback=function(on) esp.survivors=on;  task.spawn(function() espDoSurvivors(on)  end) end })
secESP:CreateToggle({ Name = "Generators", Flag="espGenerators", CurrentValue = esp.generators, Callback=function(on) esp.generators=on; task.spawn(function() espDoGenerators(on) end) end })
secESP:CreateToggle({ Name = "Items", Flag="espItems",      CurrentValue = esp.items,      Callback=function(on) esp.items=on;      task.spawn(function() espDoItems(on)      end) end })
secESP:CreateToggle({ Name = "Buildings", Flag="espBuildings",  CurrentValue = esp.buildings,  Callback=function(on) esp.buildings=on;  task.spawn(function() espDoBuildings(on)  end) end })

------------------------------------------------------------------------
-- Minion + Puddle ESP
------------------------------------------------------------------------
local secMinion = tabVisual:CreateSection("Minion & Ability ESP")
...
secMinion:CreateToggle({ Name = "c00lkidd Pizza Bots", Flag="espPizza",      CurrentValue = mset.pizza,  Callback=function(on) mset.pizza=on;  if on then scanPizza()   else clearTag("pizza")  end end })
secMinion:CreateToggle({ Name = "1x1x1x1 Zombies", Flag="espZombie",     CurrentValue = mset.zombie, Callback=function(on) mset.zombie=on; if on then scanZombie()  else clearTag("zombie") end end })
secMinion:CreateToggle({ Name = "JD Digital Footprints", Flag="espPuddle",     CurrentValue = mset.puddle, Callback=function(on) mset.puddle=on; if on then scanPuddles() else clearTag("puddle") end end })
secMinion:CreateSlider({
...
secMinion:CreateButton({ Name = "🔄 Force Rescan", Callback=function() clearTag("pizza"); clearTag("zombie"); clearTag("puddle"); task.wait(0.1); scanPizza(); scanZombie(); scanPuddles() end })
...
pcall(function()
    ...
    local secDocESP = tabVisual:CreateSection("Document / Ring ESP")
    secDocESP:CreateToggle({
        Name = "Document / Ring ESP",
        Flag = "docESPOn",
        CurrentValue = false,
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabMusic = win:CreateTab("Music", 0)
local secLMS   = tabMusic:CreateSection("LMS Music")
...
secLMS:CreateToggle({ Name = "Auto-Play on LMS", Flag="musicOn", CurrentValue = music.on, Callback=function(on) music.on=on; if on then music.thread=task.spawn(musicMonitor) else if music.thread then task.cancel(music.thread); music.thread=nil end; musicReset() end end })
secLMS:CreateDropdown({
    Name = "Track",
    Flag = "musicSel",
    Options = {},
    CurrentOption = {"Option 1"},
    MultipleOptions = false,
    Callback = function(sel) music.selected=type(sel)=="table" and sel[1] or sel; task.spawn(function()musicFetch(music.selected)end) end
})
secLMS:CreateButton({ Name = "▶  Play",        Callback=function() musicPlay(music.selected) end })
secLMS:CreateButton({ Name = "■  Stop",        Callback=function() musicReset() end })
secLMS:CreateButton({ Name = "↓  Preload LMS", Callback=function() for name in pairs(musicTracks) do task.spawn(function()musicFetch(name)end); task.wait(0.1) end end })
...
local tabElliot  = win:CreateTab("Elliot", 0)
local tabChance  = win:CreateTab("Chance", 0)

-- Elliot Aimbot
do
    local sec_014 = tabElliot:CreateSection("Elliot Aimbot")
    ...
    sec_014:CreateSlider({
    Name = "Prediction Studs",
    Flag = "elliotPredDist",
    Range = {0, 50},
    Increment = 1,
    CurrentValue = 5,
    ...
    sec_014:CreateDropdown({
    Name = "Aimbot Type",
    Flag = "elliotAimType",
    Options = {"HRP Aimbot","Camera Aimbot","Camera + Character"},
    CurrentOption = {"Camera + Character"},
    MultipleOptions = false,
    ...
    CurrentOption = {"Low HP"},
    MultipleOptions = false,
    Callback = function(v) elliotTargetMode=v end
})
    sec_014:CreateToggle({ Name = "Show Pizza Arc", Flag = "elliotShowArc", CurrentValue = false, Callback=function(v)
        elliotShowArc=v
    ...
    sec_014:CreateToggle({ Name = "Require Throw Animation", Flag = "elliotReqAnim", CurrentValue = true, Callback=function(v) elliotRequireAnim=v end})
    sec_014:CreateToggle({ Name = "Enable Elliot Aimbot", Flag = "elliotEnabled", CurrentValue = false, Callback=function(v)
        elliotEnabled = v
        if v then
...
-- Chance Aimbot
do
    local sec_020 = tabChance:CreateSection("Chance Aimbot")
    ...
    sec_020:CreateToggle({ Name = "Enable Aimbot", Flag = "chanceAimOn", CurrentValue = false, Callback=function(v) chanceAimEnabled=v end})
    sec_020:CreateDropdown({
    Name = "Prediction Mode",
    Flag = "chancePredMode",
    Options = {"Velocity","Ping","Look","LookPing"},
    CurrentOption = {"Velocity"},
    MultipleOptions = false,
    Callback = function(v) chancePredMode=v end
})
    sec_020:CreateInput({
    ...
    PlaceholderText = "0.5",
    RemoveTextAfterFocusLost = false,
    Callback = function(v) local n=tonumber(v); if n then chancePredValue=n end end
})
    sec_020:CreateSlider({
    Name = "Smooth Speed",
    Flag = "chanceSmoothSpd",
    Range = {1, 30},
    Increment = 1,
    CurrentValue = 14,
    Callback = function(v) chanceSmoothSpeed=v end
})
    sec_020:CreateToggle({ Name = "Height-Aware Aim", Flag = "chanceHeightAim", CurrentValue = true, Callback=function(v) chanceHeightAim=v end})
    sec_020:CreateDropdown({
    Name = "Aim Behavior",
    Flag = "chanceAimBehav",
    Options = {"Normal","360"},
    ...
    MultipleOptions = false,
    Callback = function(v) chanceAimBehavior=v end
})
    sec_020:CreateInput({
    Name = "Spin Duration",
    Flag = "chanceSpinDur",
    PlaceholderText = "0.5",
    RemoveTextAfterFocusLost = false,
    Callback = function(v) local n=tonumber(v); if n then chanceSpinDur=n end end
})
    sec_020:CreateToggle({ Name = "Anti Bait", Flag = "chanceAntiBait", CurrentValue = true, Callback=function(v) chanceAntiBait=v end})
    sec_020:CreateToggle({ Name = "Hold-to-Aim", Flag = "chanceHoldAim", CurrentValue = true, Callback=function(v) chanceHoldToAim=v end})
    sec_020:CreateDropdown({
    Name = "Aim Key",
    Flag = "chanceAimKey",
    Options = {"Q","E","R","T","F","G","X","C","V"},
    CurrentOption = {"Q"},
    MultipleOptions = false,
    Callback = function(v) chanceAimKey=Enum.KeyCode[v] end
})
    sec_020:CreateToggle({ Name = "Message When Aim", Flag = "chanceMsgOnAim", CurrentValue = false, Callback=function(v) chanceMsgOnAim=v end})
    sec_020:CreateInput({
    Name = "Message Text",
    Flag = "chanceMsgText",
    PlaceholderText = "...",
    RemoveTextAfterFocusLost = false,
    Callback = function(v) chanceMsgText=v end
})
end
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabVeeronica = win:CreateTab("Veeronica", 0)

local sec_022 = tabVeeronica:CreateSection("Auto Trick")
...
do
    ...
    sec_022:CreateToggle({
        Name = "Auto Trick", Flag = "veeeAutoTrick", CurrentValue = false, Callback = function(on)
            atEnabled = on
            if on then
...
-- SK8 Control
------------------------------------------------------------------------
local sec_023 = tabVeeronica:CreateSection("SK8 Control")
...
do
    ...
    sec_023:CreateToggle({
        Name = "Enable SK8 Control", CurrentValue = sk8_controlEnabled, Flag = "sk8ControlEnabled", Callback = function(on)
...
------------------------------------------------------------------------
local sec_vee_spray = tabVeeronica:CreateSection("Fast Spray")
...
do
    ...
    sec_vee_spray:CreateToggle({
        Name = "Enable Fast Spray", Flag = "veeeFastSpray", CurrentValue = vee_fastSpray,
        Callback = function(on)
            vee_fastSpray = on
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabJaneDoe  = win:CreateTab("Jane Doe", 0)
...
do
    ...
    local sec_024 = tabJaneDoe:CreateSection("Crystal Auto-Fire")
    sec_024:CreateToggle({ Name = "Enable Jane Doe Aimbot", Flag = "jdEnabled", CurrentValue = false,
        Callback=function(on)
            jd_enabled=on; local actor=jd_getLocalActor()
            if on and not jd_patched and actor then jd_applyPatch(actor) end
        end})
    sec_024:CreateToggle({ Name = "Aimbot (Silent Aim)", Flag = "jdSilentAim", CurrentValue = false,
        Callback=function(on)
            jd_aimbotOn=on
            local actor=jd_getLocalActor(); if on and not jd_patched and actor then jd_applyPatch(actor) end
        end})
    sec_024:CreateSlider({
    Name = "Aim Offset (Y)",
    Flag = "jdAimOffset",
    Range = {-5.0, 5.0},
    Increment = 0.1,
    ...
    local sec_025 = tabJaneDoe:CreateSection("Axe Lock-On")
    sec_025:CreateToggle({ Name = "Enable Axe Lock-On", Flag = "jdAxeEnabled", CurrentValue = false,
        Callback=function(on) jd_axeEnabled=on end})
    sec_025:CreateSlider({
    ...
    local sec_026 = tabJaneDoe:CreateSection("Control")
    sec_026:CreateButton({ Name = "Unload Jane Doe", Callback=function()
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabDusekkar = win:CreateTab("Dusekkar", 0)

do
    local sec_027 = tabDusekkar:CreateSection("PlasmaBeam Silent Aim")
    ...
    sec_027:CreateToggle({
        Name = "Enable PlasmaBeam Aim", CurrentValue = plasma_enabled, Flag = "plasmaEnabled",
        Callback = function(on) plasma_enabled = on end })
    sec_027:CreateSlider({
    Name = "Prediction (s)",
    Flag = "plasmaPrediction",
    Range = {0.0, 0.5},
    Increment = 0.01,
    CurrentValue = 0,
    ...
    sec_027:CreateSlider({
    Name = "Aim Height Offset",
    Flag = "plasmaAimOffset",
    Range = {-5.0, 5.0},
    Increment = 0.1,
    CurrentValue = plasma_aimOffset,
    Callback = function(v) plasma_aimOffset = v end
})

    local sec_028 = tabDusekkar:CreateSection("Control")
    sec_028:CreateButton({
        Name = "Unload PlasmaBeam Hook", Callback = function()
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabNoli = win:CreateTab("Noli", 0)

do
    local sec_029 = tabNoli:CreateSection("Nova Silent Aim")
    ...
    sec_029:CreateToggle({
        Name = "Enable Nova Aim", CurrentValue = nova_enabled, Flag = "novaEnabled",
        Callback = function(on) nova_enabled = on end })
    sec_029:CreateSlider({
    Name = "Prediction (s)",
    Flag = "novaPrediction",
    Range = {0.0, 0.5},
    Increment = 0.01,
    CurrentValue = nova_prediction,
    ...
    Range = {-5.0, 5.0},
    Increment = 0.1,
    CurrentValue = nova_aimOffset,
    Callback = function(v) nova_aimOffset = v end
})

    local sec_029b = tabNoli:CreateSection("Control")
    sec_029b:CreateButton({
        Name = "Unload Nova Hook", Callback = function()
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabAI = win:CreateTab("AI", 0)
local secAIMain = tabAI:CreateSection("Killer AI")

secAIMain:CreateLabel("What this does: Pathfinds to the nearest survivor using PathfindingService. Killer-only — switch to killer before enabling.")
...
secAIMain:CreateToggle({
    Name = "Enable Killer AI Farm", Flag = "aiKillerEnabled", CurrentValue = ai_enabled,
    Callback = function(on)
        ai_enabled = on
        if on then
...
secAIMain:CreateToggle({
    Name = "Auto Reset on Death", Flag = "aiResetOnDeath", CurrentValue = ai_resetOnDeath,
    Callback = function(on) ai_resetOnDeath = on end
})

local secAICtrl = tabAI:CreateSection("Control")
secAICtrl:CreateButton({
    Name = "Stop AI", Callback = function()
        ai_enabled = false
        if ai_thread then task.cancel(ai_thread); ai_thread = nil end
        if aiState.moveConn then aiState.moveConn:Disconnect(); aiState.moveConn = nil end
        aiState.target    = nil
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabGuest1337 = win:CreateTab("Guest 1337", 0)

-- GUEST1337 — Auto Block & Combat
------------------------------------------------------------------------
local sec_015 = tabGuest1337:CreateSection("Auto Block & Combat")
...
-- UI Elements
sec_015:CreateToggle({ Name = "Auto Block (Audio)", Flag = "combatAutoBlock", CurrentValue = combatS.autoBlockOn, Callback=function(on) 
        combatS.autoBlockOn=on
        if on then combatSetupSoundWatcher(); combatStartLoops()
        else combatStopLoops() end
    end})

sec_015:CreateDropdown({
    Name = "Block Type",
    Flag = "combatBlockType",
...
sec_015:CreateSlider({
    Name = "Detection Range",
    Flag = "combatDetRange",
    Range = {5, 50},
    Increment = 1,
    CurrentValue = combatS.detectionRange,
...
sec_015:CreateSlider({
    Name = "Block Delay (s)",
    Flag = "combatBlockDelay",
    Range = {0, 0.5},
    Increment = 0.01,
    CurrentValue = combatS.blockDelay,
    Callback = function(v) combatS.blockDelay=v end
})

sec_015:CreateToggle({ Name = "Double Block Tech", Flag = "combatDoubleBlock", CurrentValue = combatS.doubleBlock, Callback=function(on) combatS.doubleBlock=on end})

sec_015:CreateToggle({ Name = "Anti-Bait", Flag = "combatAntiBait", CurrentValue = combatS.antiBait, Callback=function(on) combatS.antiBait=on end
})

sec_015:CreateSlider({
    Name = "Block Miss Chance %",
    Flag = "combatMissChance",
    Range = {0, 100},
    Increment = 1,
    CurrentValue = 0,
...
local sec_016 = tabGuest1337:CreateSection("Auto Punch")

sec_016:CreateToggle({ Name = "Auto Punch", Flag = "combatAutoPunch", CurrentValue = combatS.autoPunchOn, Callback=function(on) combatS.autoPunchOn=on end
})

local sec_017 = tabGuest1337:CreateSection("HDT (Hitbox Dragging)")


sec_017:CreateToggle({ Name = "Enable HDT", Flag = "combatHDT", CurrentValue = combatS.hdtEnabled, Callback=function(on) combatS.hdtEnabled=on end})

sec_017:CreateSlider({
    Name = "Sprint Speed",
    Flag = "combatHDTMoveSpeed",
    Range = {1, 100},
    Increment = 1,
    CurrentValue = 0,
...
sec_017:CreateSlider({
    Name = "Drag Duration (s)",
    Flag = "combatHDTDur",
    Range = {0.1, 3.0},
    Increment = 0.1,
    CurrentValue = combatS.hdtDuration,
    Callback = function(v) combatS.hdtDuration=v end
})

sec_017:CreateSlider({
    Name = "HDT Miss Chance %",
    Flag = "combatHDTMiss",
    Range = {0, 100},
    Increment = 1,
    CurrentValue = 0,
...
local sec_018 = tabGuest1337:CreateSection("Vision")

sec_018:CreateToggle({ Name = "Detection Circles", Flag = "combatCircles", CurrentValue = combatS.killerCircles, Callback=function(on) 
        combatS.killerCircles=on
        if on then combatStartLoops() else combatUpdateCircles() end
    end})

sec_018:CreateToggle({ Name = "Facing Check", Flag = "combatFacingCheck", CurrentValue = combatS.facingCheck, Callback=function(on) combatS.facingCheck=on end})

sec_018:CreateToggle({ Name = "Facing Visual", Flag = "combatFacingVis", CurrentValue = combatS.facingVisual, Callback=function(on)
        combatS.facingVisual=on
        if on then combatStartLoops() end
    end})

sec_018:CreateSlider({
    Name = "Facing Visual Size",
    Flag = "combatFacingSize",
    Range = {1, 10},
    Increment = 0.5,
    CurrentValue = combatS.facingVisRadius,
    Callback = function(v)
...
local sec_019 = tabGuest1337:CreateSection("Aim Punch Lock")

sec_019:CreateToggle({ Name = "Aim Punch", Flag = "combatAimPunch", CurrentValue = combatS.aimPunchActive,
    Callback = function(on)
...
sec_019:CreateSlider({
    Name = "Punch Prediction",
    Flag = "combatPunchPred",
    Range = {0, 10},
    Increment = 0.1,
    CurrentValue = combatS.punchPrediction,
...
sec_019:CreateSlider({
    ...
    Range = {0.1, 2.0},
    Increment = 0.05,
    CurrentValue = combatS.aimPunchDuration,
...
local tabTT  = win:CreateTab("Two-Time", 0)
local secTT  = tabTT:CreateSection("Dagger Config")

secTT:CreateToggle({ Name = "Enabled", Flag = "ttEnabled", CurrentValue = false,
...
secTT:CreateToggle({ Name = "Show Circle", Flag = "ttCircle", CurrentValue = true,
    Callback = function(v)
        ttS.showCircle = v
        if not v then
...
secTT:CreateToggle({ Name = "Noclip Killers", Flag = "ttNoclip", CurrentValue = false,
    Callback = function(v)
        ttS.noclipKillers = v
        if not v then
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabInterface = win:CreateTab("Interface", 0)
local sec_030 = tabInterface:CreateSection("UI Functions")

sec_030:CreateButton({ Name = "Close UI", Callback = function()
...
-- Rayfield loads config and fires all callbacks automatically
