...
-- run in a separate thread so WaitForChild never blocks script execution
task.spawn(function()
    local ok, err = pcall(hookCrouch)
    if not ok then print("[SAKI-TT] hookCrouch error:", tostring(err)) end
end)
