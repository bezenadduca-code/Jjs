...
local categoriesSection = tabESP:Section({ Title = "Categories", Opened = true })
...
local visualSection = tabESP:Section({ Title = "Visual Settings", Opened = true })
...
local CONFIG_PATH = "v1prware/config.json"

secConfigShare:Button({
    Title = "📋 Copy Config (Shareable)",
    Icon = "copy",
    Callback = function()
        local ok, err = pcall(function()
            if not fs.hasFile(CONFIG_PATH) then
                print("[v1prware] No config file found to copy")
                return
            end
            local raw = fs.read(CONFIG_PATH)
            setclipboard("v1prw_full:" .. b64Encode(raw))
            print("[v1prware] Full config copied to clipboard!")
        end)
        if not ok then
            print("[v1prware] Copy failed: " .. tostring(err))
        end
    end
})

local importConfigStr = ""
...
-- Short code restore function
local function restoreShortCode(code)
    if not code or not code:match("^V1PRW:") then return nil end
    local chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    local compressed = ""
    local charPart = code:sub(7)
    
    for i = 1, #charPart do
        local val = (string.find(chars, charPart:sub(i, i)) or 1) - 1
        for j = 0, 3 do
            compressed = compressed .. (math.floor(val / math.pow(2, 3 - j)) % 2 == 1 and "1" or "0")
        end
    end
    return compressed
end

-- Function to apply config from flags
local function applyConfigFromFlags()
    task.wait(0.5)
    ...
    -- Spoof
    if win.Flags.spoofActive ~= nil then
        spoofActive = win.Flags.spoofActive
        if spoofActive then spoofStart() else spoofStop() end
        print("[v1prware] Spoof set to: " .. tostring(spoofActive))
    end
    if win.Flags.spoofText ~= nil then
        spoofText = win.Flags.spoofText
    end
    
    -- Chat Force
    if win.Flags.chatForceEnabled ~= nil then
        chatForceEnabled = win.Flags.chatForceEnabled
        if chatForceConn then chatForceConn:Disconnect(); chatForceConn = nil end
        if chatForceEnabled then
            enforceChatOn()
            chatForceConn = svc.Run.Heartbeat:Connect(enforceChatOn)
        end
        print("[v1prware] Chat Force set to: " .. tostring(chatForceEnabled))
    end
    
    -- Timer Position
    if win.Flags.timerSide ~= nil then
        if type(win.Flags.timerSide) == "table" then
            timerSide = win.Flags.timerSide[1] or "Middle"
        else
            timerSide = win.Flags.timerSide
        end
        applyTimerPos()
    end
    
    -- Platform Spoofer
    if win.Flags.platEnabled ~= nil then
        platEnabled = win.Flags.platEnabled
        if platEnabled then platStart() else platStop() end
    end
    if win.Flags.platDevice ~= nil then
        platDevice = win.Flags.platDevice
    end
    
    -- Stamina
    if win.Flags.stamOn ~= nil then 
        stam.on = win.Flags.stamOn 
        print("[v1prware] Stamina On set to: " .. tostring(stam.on))
    end
    if win.Flags.stamNoLoss ~= nil then 
        stam.noLoss = win.Flags.stamNoLoss 
    end
    ...
    -- Speed Hack
    if win.Flags.speedOn ~= nil then
        speedHack.on = win.Flags.speedOn
        if speedHack.on then speedStart() end
    end
    if type(win.Flags.speedValue) == "number" then
        speedHack.speed = win.Flags.speedValue
        speedHack.lastApplied = 0
    end
    
    -- Hitbox
    if win.Flags.hbOn ~= nil then
        hb.on = win.Flags.hbOn
        if hb.on then hbStart() else hbStop() end
    end
    if type(win.Flags.hbStrength) == "number" then hb.strength = win.Flags.hbStrength end
    
    -- Auto Collision
    if win.Flags.acOn ~= nil then
        ac.on = win.Flags.acOn
        if not ac.on then
            for k in pairs(ac.active) do ac.active[k] = nil end
            ac.chaseTarget = nil; ac.damageTarget = nil
        end
    ...
    -- Auto Solve
    if win.Flags.flowOn ~= nil then 
        flow.on = win.Flags.flowOn 
        print("[v1prware] Flow On set to: " .. tostring(flow.on))
    end
    ...
    -- Aimbot
    if win.Flags.aimOn ~= nil then 
        aim.on = win.Flags.aimOn 
        print("[v1prware] Aimbot On set to: " .. tostring(aim.on))
    end
    ...
    -- Anti-Backstab
    if win.Flags.absOn ~= nil then
        abs.on = win.Flags.absOn
    ...
    -- Noli Void Rush
    if win.Flags.noliVoidRushOn ~= nil then noliVoidRushOn = win.Flags.noliVoidRushOn end
    if not noliVoidRushOn then noliStop() end
    
    -- ESP Main
    if win.Flags.espEnabled ~= nil then
        esp.enabled = win.Flags.espEnabled
        if esp.enabled then espStart() else espStop() end
        print("[v1prware] ESP Enabled set to: " .. tostring(esp.enabled))
    end
    ...
    -- Music
    if win.Flags.musicOn ~= nil then
        music.on = win.Flags.musicOn
        if music.on then
            if music.thread then task.cancel(music.thread) end
            music.thread = task.spawn(musicMonitor)
        else
        ...
        print("[v1prware] Music On set to: " .. tostring(music.on))
    end
    if win.Flags.musicSel ~= nil then
        music.selected = win.Flags.musicSel
    end
...
secConfigShare:Button({
    Title = "🔄 Load Config From String",
    Icon = "download",
    Callback = function()
        if importConfigStr == "" then
            print("[v1prware] Paste a config string first")
            return
        end
        
        print("[v1prware] Loading config from string...")
        
        -- Handle short code (V1PRW:)
        if importConfigStr:match("^V1PRW:") then
            local decoded = restoreShortCode(importConfigStr)
            if decoded then
        ...
        -- Handle full code (v1prw_full:)
        if importConfigStr:match("^v1prw_full:") then
            local ok, err = pcall(function()
                -- Decode base64
                local decoded = b64Decode(importConfigStr:sub(12))
                print("[v1prware] Decoded config length: " .. #decoded)
                
                -- Write to file
                fs.write(CONFIG_PATH, decoded)
                print("[v1prware] Config written to: " .. CONFIG_PATH)
                
                -- Load the config
                cfg.load()
                print("[v1prware] Config loaded from file")
                
                -- Apply flags
                applyConfigFromFlags()
            end)
            if ok then
                print("[v1prware] Full config loaded successfully!")
            else
                print("[v1prware] Load failed: " .. tostring(err))
            end
            return
        end
...
secConfigShare:Button({
    ...
    Callback = function()
        cfg.save()
        print("[v1prware] Config saved to " .. CONFIG_PATH)
    end
})
...
secConfigShare:Button({
    ...
    Callback = function()
        cfg.load()
        print("[v1prware] Config loaded from " .. CONFIG_PATH)
        applyConfigFromFlags()
    end
})
