...
do
    ...
    local function jd_holdCrystal()
        if not jd_RemoteEvent then return end
        local username = jd_lp.Name
        jd_RemoteEvent:FireServer(username .. "CrystalInput", {
            buffer.fromstring("\x02\x00\x00\x00\xBA\xB3\x1E\xDA?")
        })
    end
    ...
    -- Hook namecall to detect Axe remote
    task.spawn(function()
        local ok, re = pcall(function()
        ...
        oldNC = hookmetamethod(game, "__namecall", function(self, ...)
            ...
            if method == "FireServer" and self == re then
                if args[1] == "UseActorAbility" and args[2] and args[2][1] then
                    local ok2, bs = pcall(function() return buffer.tostring(args[2][1]) end)
                    if ok2 and bs and bs:find("Axe") and jd_axeEnabled then
    ...
    -- Hook: detect CrystalInput, spam hold remote for HOLD_DURATION then stop
    ...
    task.spawn(function()
        ...
        if not ok or not re then return end
        local oldNC2
        oldNC2 = hookmetamethod(game, "__namecall", function(self, ...)
            local method = getnamecallmethod()
            local args = {...}
            if method == "FireServer" and self == re and jd_enabled then
                local ok2, bs = pcall(function()
                    if args[2] and args[2][1] then return buffer.tostring(args[2][1]) end
                    return tostring(args[1])
                end)
                if ok2 and bs and (bs:find("CrystalInput") or tostring(args[1]):find("CrystalInput")) then
